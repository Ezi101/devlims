    ...
    public function modifyAdminMenu()
    {
        ...
        if ($is_essentials_enabled) {
            Menu::modify('admin-sidebar-menu', function ($menu) {
                if (auth()->user()->can('Modules.hrm.view')) {
                    $menu->url(
                        action([\Modules\Essentials\Http\Controllers\DashboardController::class, 'hrmDashboard']),
                        __('essentials::lang.hrm'),
                        ['icon' => 'fa fas fa-users text-color-21', 'active' => request()->segment(1) == 'hrm', 'style' => config('app.env') == 'demo' ? 'background-color: #605ca8 !important;' : '']
                    )
                        ->order(87);
                }
                ...
                $menu->url(
                    ...
                    ['icon' => 'fa fas fa-check-circle text-color-22', 'active' => request()->segment(1) == 'essentials', 'style' => config('app.env') == 'demo' ? 'background-color: #001f3f !important;' : '']
                )
                    ->order(87);
            });
        }
