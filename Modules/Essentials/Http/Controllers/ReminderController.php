    ...
    public function store(Request $request)
    {
        ...
        if ($request->ajax()) {
            try {
                ...
                \Log::info('Reminder Data: ', $reminder);
    
                $reminder = Reminder::create($reminder);
                \Log::info('Created Reminder: ', $reminder->toArray());
    
                $reminder_time = Carbon::parse($reminder['date'] . ' ' . $reminder['end_time']);
    
                $current_time = Carbon::now();
    
                if ($reminder_time->greaterThan($current_time)) {
                    $delay = $reminder_time->diffInSeconds($current_time);
                    $user = auth()->user();
                    SendReminderNotification::dispatch($user, $reminder)->delay($delay);
                }
