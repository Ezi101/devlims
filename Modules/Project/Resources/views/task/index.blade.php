@if ($can_crud_task || $is_lead_or_admin)
    ...
    @if (auth()->user()->can('workflow.add_task'))
        <button type="button" class="btn btn-sm btn-primary task_btn pull-right m-5"
            data-href="{{ action([\Modules\Project\Http\Controllers\TaskController::class, 'issue_test'], ['project_id' => $project->id]) }}">
            @lang('messages.add_test')&nbsp;
            <i class="fa fa-plus"></i>
        </button>
    @endif
@endif
...
<div class="btn-group btn-group-toggle pull-right m-5" data-toggle="buttons">
    <label class="btn btn-info btn-sm 
        @if (
            (!empty($project->settings) && !isset($project->settings['task_view'])) ||
                (isset($project->settings['task_view']) && $project->settings['task_view'] == 'list_view')) active @endif">
        <input type="radio" name="task_view" value="list_view" class="task_view"
            @if (
                (!empty($project->settings) && !isset($project->settings['task_view'])) ||
                    (isset($project->settings['task_view']) && $project->settings['task_view'] == 'list_view')) checked @endif>
        @lang('project::lang.list_view')
    </label>
    <label class="btn btn-info btn-sm
        @if (isset($project->settings['task_view']) && $project->settings['task_view'] == 'kanban') active @endif">
    ...
    <table class="table table-bordered table-striped" id="project_task_table">
        <thead>
            <tr>
                ...
                <th class="col-md-3"> @lang('project::lang.test/task_des')</th>
                ...
                <th>@lang('project::lang.duration')</th>
