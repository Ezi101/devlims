...
$('#project_model').on('shown.bs.modal', function (e) {
    ...
    $('form#project_form .datetimepicker').datetimepicker({
        ignoreReadonly: true,
...
// model intiliazing on sample dashbord

$(document).on('click', 'button.add_new_workflow', function () {
    var url = $(this).data('href');
    var id = $(this).data('id');
    var dataToSend = {
        id: id
    ...
    $.ajax({
        ...
        data: dataToSend,
        success: function (result) {
            $('#new_workflow_model').html(result).modal("show");
        }
    });
});


//initialize ck editor, date picker and form validation when WorkFlow form Sample Dashbord model is opened
$('#new_workflow_model').on('shown.bs.modal', function (e) {

    $('form#workflow_form .datepicker').datepicker({
        autoclose: true,
        format: datepicker_date_format
    });

    $('form#workflow_form .datetimepicker').datetimepicker({
        ignoreReadonly: true,
        format: moment_date_format + ' ' + moment_time_format
    });
    ...
    $(".select2").select2();
    //form validation
    $("form#workflow_form").validate();
});

$('#new_workflow_model').on('hidden.bs.modal', function () {
    tinymce.remove("textarea#description");
});

//WorkFlow form Sample Dashbord form submit
$(document).on('submit', 'form#workflow_form', function (e) {
    e.preventDefault();
    var url = $('form#workflow_form').attr('action');
    var method = $('form#workflow_form').attr('method');
    var data = $('form#workflow_form').serialize();
    var ladda = Ladda.create(document.querySelector('.ladda-button'));
    ladda.start();
    $.ajax({
        method: method,
...
/**
 * Workflow  MODULE Form Sample Dashbord
 * Workflow  task Form Sample Dashbord related code
 */
// Workflow  task Form Sample Dashbord - add form model ON Sample Dashbord
$(document).on('click', '.from_sample_dashbord_task_btn', function () {
    var url = $(this).data('href');
    var id = $(this).data('id');
    var dataToSend = {
        id: id
    ...
    $.ajax({
        ...
        data: dataToSend,
        success: function (result) {
            $('.sample_dashbord_project_task_model').html(result).modal("show");
        }
    });
});

//initialize ck editor, date picker and form validation when model is opened
$('.sample_dashbord_project_task_model').on('shown.bs.modal', function (e) {

    tinymce.init({
        selector: 'textarea#description',
    });

    $('form#sample_dashbord_task_form .datepicker').datepicker({
        autoclose: true,
        format: datepicker_date_format
    });
    ...
    $(".select2").select2();
    //form validation
    $("form#sample_dashbord_task_form").validate();
...
$('.sample_dashbord_project_task_model').on('hidden.bs.modal', function () {
    tinymce.remove("textarea#description");
});

//project task form submit
$(document).on('submit', 'form#sample_dashbord_task_form', function (e) {
    e.preventDefault();
    var url = $('form#sample_dashbord_task_form').attr('action');
    var method = $('form#sample_dashbord_task_form').attr('method');
    var data = $('form#sample_dashbord_task_form').serialize();
    var ladda = Ladda.create(document.querySelector('.ladda-button'));
    ladda.start();
    $.ajax({
        method: method,
        ...
        success: function (result) {
            ladda.stop();
            if (result.success) {
                $('.sample_dashbord_project_task_model').modal("hide");
                toastr.success(result.msg);
