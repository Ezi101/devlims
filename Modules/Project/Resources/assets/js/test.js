$(document).ready(function () {
    ...
    $('#supplier_id')
        .select2({
            ajax: {
                url: '/receive-stock/get_suppliers',
                dataType: 'json',
                delay: 250,
                    ...
                    if (ui.content.length == 1) {
                        ...
                        swal({
                            ...
                            if (value) {
                                ...
                                $.ajax({
                                    url: '/samples/quick_add?product_name=' + term,
                                    dataType: 'html',
                                    success: function (result) {
...
// $('#search_product_by_issue_id').on('input', function() {
//     if ($('#search_product_by_issue_id').length > 0) {
//         var data = {
//             term: $('#search_product_by_issue_id').val()
//         };
//         $('#search_product_by_issue_id').autocomplete({
//             source: function(request, response) {

//                 $.ajax({
//                     method: 'POST',
//                     url: '/receive-stock/get_stock_entry_',
//                     data: data,
//                     success: function (result) {
//                         console.log(result, );
//                         $('#product_name').append(result["transaction"]["product"]["name"]);
//                         $('#product_type').append(result["transaction"]["product"]["product_type"]);
//                         $('#product_issue_id').append(result["transaction"]["invoice_no"]);
//                         $('#product_batch_no').append(result["transaction"]["batch_no"]["code"]);
//                         // $('#product_quantity').append(result["product"]["product_type"]);
...
// CODE FOR SAMPLE TEST REPORT TO GET THE BATCH ID ON SLECTING SAMPLE

// Event handler when a sample is selected
$('#sample').on('change', function () {
    var selectedSampleId = $(this).val();
    if (selectedSampleId) {
        $.ajax({
            method: 'POST',
            url: '/recevie-stock/get_batches_by_sample',
            data: { sample_id: selectedSampleId },
            success: function (result) {
                var batches = result.batches;
                var wbatches = result.wtransaction;
                var batchSelectId = '#batch';
                var wbatchSelectId = '#wbatch';
                var wbatchfield = '#water_batch_field';
                var ptr_no_for_str = '#ptr_no_for_str';
                var ptr_status_for_str = '#ptr_status_for_str';

                $(batchSelectId).empty().append('<option selected disabled>Please Select</option>');
                $(wbatchSelectId)
                    .empty()
                    .append('<option selected disabled>Please Select</option>');

                // 🔹 Handle error cases without reload
                if ((!batches || batches.length === 0) && !result.ptr) {
                    swal({
                        icon: 'error',
                        title: 'Data Missing',
                        text: 'No batches or PTR found for the selected sample!',
                    });
                    $(ptr_status_for_str).val('');
                    $(ptr_no_for_str).val('');
                    return;
                    ...
                    swal({
                        ...
                        text: 'No batches found for the selected sample!',
                    });
                    $(batchSelectId).prop('disabled', true).addClass('disable-class');
                    return;
                } else if (!result.ptr) {
                    swal({
                        icon: 'error',
                        title: 'No active PTR Found',
                        text: 'No active PTR found for the selected sample!',
                    });
                    $(ptr_status_for_str).val('');
                    $(ptr_no_for_str).val('');
                    return;
                ...
                if (batches && batches.length > 0) {
                    batches.forEach(function (batch) {
                        var optionText = `${batch.batch_code || batch.batch_no}`;
                        if (batch.transaction_id) {
                            let installmentText = '';
                            switch (batch.installments) {
                                case 'instalments_1':
                                    installmentText = '1st';
                                    break;
                                case 'instalments_2':
                                ...
                                case 'instalments_1_2':
                                    installmentText = '1st & 2nd';
                                    break;
                                case 'instalments_1_2_3':
                                    installmentText = '1st,2nd & 3rd';
                                    break;
                                case 'instalments_2_3':
                                    installmentText = '2nd & 3rd';
                                    break;
                                case 'instalments_3_4':
                                    installmentText = '3rd & 4th';
                                    break;
                                case 'no_instalment':
                            ...
                            optionText += ` - ${installmentText} - (${batch.reference_no})`;
                        }
                        $(batchSelectId).append(
                            $('<option>', {
                    ...
                    $(batchSelectId).prop('disabled', false).removeClass('disable-class');
                }

                // 🔹 Populate water batches
                if (wbatches && wbatches.length > 0) {
                    wbatches.forEach(function (wbatch) {
                        $(wbatchSelectId).append(
                            $('<option>', {
                                value: wbatch.id,
                                text: wbatch.code,
                            })
                    ...
                    $(wbatchfield).show();
                    $(wbatchSelectId).prop('disabled', false).removeClass('disable-class');
                } else {
                    $(wbatchfield).hide();
                    $(wbatchSelectId).prop('disabled', true).addClass('disable-class');
                }

                // 🔹 PTR values
                if (result.ptr) {
                    $(ptr_status_for_str).val(result.ptr.status);
                    $(ptr_no_for_str).val(result.ptr.ptr_no);
                } else {
                    $(ptr_status_for_str).val('');
                    $(ptr_no_for_str).val('');
                }
            ...
            error: function (xhr, status, error) {
                ...
                // Do not reload, just reset fields
                $('#batch').prop('disabled', true).addClass('disable-class');
                $('#wbatch').prop('disabled', true).addClass('disable-class');
                $('#water_batch_field').hide();
                $('#ptr_no_for_str').val('');
                $('#ptr_status_for_str').val('');
            },
        });
...
$(document).ready(function () {
    ...
    $('#search_product_by_issue_id').off('input');

    // Add a click event listener for the search button
    $('.search_button').on('click', function () {
        var searchTerm = $('#search_product_by_issue_id').val();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (
                    ...
                    // Display a SweetAlert message if the sample is not found
                    swal({
                        title: 'Data Not found',
                        text: 'Invalid Issue ID: ' + data.term,
                        buttons: ['Cancel', 'OK'],
                    }).then((value) => {
                        ...
                        if (value === 'OK') {
                            ...
                            // Handle Cancel button click or do nothing
                        }
                    });
                    return;
                }
                ...
                var selectId = '#test_name';
                $(selectId).empty();
                $(selectId).append('<option selected disabled>Please select</option>');

                for (var i = 0; i < tests.length; i++) {
                    $(selectId).append(
                        $('<option>', {
                            value: tests[i].test_id,
                            text: tests[i].testmethod.name,
                        })
                    );
                }

                // Append other data to different divs as needed
                $('#product_name').html(result.transaction.product.name);
                $('#product_type').html(result.transaction.product.product_type);
                $('#product_issue_id').html(result.transaction.invoice_no);
                $('#product_batch_no').html(result.transaction.batch_no.code);
            },
        });
    });

    // Unbind any existing change event handler for 'test_name' select element
    $(document).off('change', '#test_name');

    // Add a change event handler for the 'test_name' select element
    $(document).on('change', '#test_name', function () {
        var selectedTestName = $(this).find('option:selected').text();
        $('#subject').val(selectedTestName);
    });
});
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        $.ajax({
            method: 'POST',
            url: '/receive-stock/get_stock_entry_row',
            data: data,
            success: function (result) {
                console.log(result);
                // console.log(result["product"]["name"]);
                // console.log("Product ID: " + result.product.name);
...
function append_purchase_lines(data, row_count, trigger_change = false) {
    ...
    console.log(dataAppended);
    if (!dataAppended) {
        console.log(data);

        // Assuming data["product"] is a string, append it to a div with the id 'myDiv'
        $('#product_name').append(data['product']['name']);
        $('#product_type').append(data['product']['product_type']);
        $('#product_issue_id').append(data['product']['product_type']);
        $('#product_batch_no').append(data['product']['product_type']);
        $('#product_quantity').append(data['product']['product_type']);

        // Set the flag to true to indicate that data has been appended
        dataAppended = true;
    }
    ...
    $(data)
        ...
        .each(function () {
            ...
            $('#s_details').append(update_purchase_entry_row_values(row));
...
$(document).on('click', 'button#submit_purchase_form', function (e) {
    ...
    $('form#add_purchase_form').validate({
        rules: {
            ref_no: {
                remote: {
                    url: '/receive-stock/check_ref_number',
                    type: 'post',
                    data: {
                        ref_no: function () {
                            return $('#ref_no').val();
...
$(document).on('change', '#supplier_id', function () {
    if ($('#purchase_order_ids').length) {
        contact_id = $(this).val();
        $.ajax({
            url: '/get-receive-stock-orders/' + contact_id,
            dataType: 'json',
...
$('#purchase_order_ids').on('select2:select', function (e) {
    ...
    var row_count = $('#row_count').val();
    $.ajax({
        url: '/get-receive-stock-order-lines/' + purchase_order_id + '?row_count=' + row_count,
        dataType: 'json',
        success: function (data) {
...
if ($('div#import_product_dz').length) {
    $('div#import_product_dz').dropzone({
        url: base_path + '/import-receive-stock-samples',
        paramName: 'file',
