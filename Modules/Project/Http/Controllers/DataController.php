...
class DataController extends Controller
{
    ...
    public function modifyAdminMenu()
    {
        ...
        if ($is_project_enabled) {
            Menu::modify(
                'admin-sidebar-menu',
                function ($menu) {
                    if (auth()->user()->can('Modules.project.view')) {

                        $menu->url(
                            action([\Modules\Project\Http\Controllers\ProjectController::class, 'index']) . '?project_view=list_view',
                            __('project::lang.project'),
                            ['icon' => 'fa fa-project-diagram text-color-20', 'active' => request()->segment(1) == 'project' || request()->get('type') == 'project', 'style' => config('app.env') == 'demo' ? 'background-color: #e4186d !important;' : '']
                        )
                            ->order(86);
                    }
                }
    ...
    public function user_permissions()
    {
        return [
            [
                'value' => 'workflow.create_project',
                'label' => __('project::lang.create_project'),
                'default' => false,
            ],
            [
                'value' => 'workflow.edit_project',
                'label' => __('project::lang.edit_project'),
                'default' => false,
            ],
            [
                'value' => 'workflow.delete_project',
                'label' => __('project::lang.delete_project'),
                'default' => false,
            ],
            [
                'value' => 'workflow.add_task',
                'label' => __('project::lang.add_task'),
                'default' => false,
            ],
            [
                'value' => 'workflow.shortcut_create workflowand_issue_tests',
                'label' => __('project::lang.shortcut_create workflowand_issue_tests'),
                'default' => false,
            ],
        ];
    }
