...
class ProjectController extends Controller
{
    ...
    public function create()
    {
        ...
        $samples = Product::forDropdown($business_id, false);
        $customers = Contact::customersDropdown($business_id, false);
        $statuses = Project::statusDropdown();
        $categories = ProjectCategory::forDropdown($business_id, 'project');

        return view('project::project.create')
            ->with(compact('users', 'customers', 'statuses', 'categories', 'samples'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $input = $request->only('name', 'description', 'product_id', 'contact_id', 'status', 'lead_id');
            $input['start_date'] = !empty($request->input('start_date')) ? $this->commonUtil->uf_date($request->input('start_date'), true) : null;
            $input['end_date'] = !empty($request->input('end_date')) ? $this->commonUtil->uf_date($request->input('end_date'), true) : null;
            ...
            DB::commit();
            AuditLogger::log('created', 'Workflow', 'Test ID: ' . $project->id);
    ...
    public function edit($id)
    {
        ...
        if (!(auth()->user()->can('superadmin') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'project_module') && auth()->user()->can('workflow.edit_project')))) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('project::project.edit')
            ->with(compact('users', 'customers', 'statuses', 'project', 'categories', 'samples'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if (!empty($project_members['attached'])) {
                ...
                // dd($project->id,$project);
                $project['link'] = route('project.show', $project->id);
    ...
    public function destroy($id)
    {
        ...
        if (!(auth()->user()->can('superadmin') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'project_module') && auth()->user()->can('workflow.delete_project')))) {
            abort(403, 'Unauthorized action.');
        }
