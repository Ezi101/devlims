...
class TaskController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $project_task = ProjectTask::with(['members', 'samplereading', 'createdBy', 'project', 'comments'])
                ->where('business_id', $business_id)
                ->select('*');
            //     $tasks = $project_task->get();
            // dd($tasks);
            ...
            if (request()->get('task_view') == 'list_view') {
                return Datatables::of($project_task)
                    ...
                    ->editColumn('test_id', function ($row) {
                        return $row->samplereading?->test;
                    })
                    ->editColumn('duration', function ($row) {
                        $startDate = $row->start_date ? Carbon::parse($row->start_date)->startOfDay() : null;
                        $now = Carbon::now();
                        if ($row->status == 'completed' || $row->status == 'canceled') {
                            return ucfirst($row->status);
                        } elseif ($startDate) {
                            $assignedDays = $now->diffInDays($startDate);
                            return $assignedDays;
                        }
                    })
    ...
    public function issue_test()
    {
        ...
        $priorities = ProjectTask::prioritiesDropdown();
        $statuses = ProjectTask::taskStatuses();
        $devices = Instruments::pluck('name', 'id');

        // Modify the device array to include both ID and name
        foreach ($devices as $key => $value) {
            $devices[$key] = $key . ' - ' . $value; // Concatenate ID and name
        }
        // code for issue test in task controller
        ...
        return view('project::task.issue_test')
            ->with(compact('project_members', 'priorities', 'devices', 'project_id', 'statuses', 'default_location', 'test_status', 'business_locations'));
    }

    public function issue_test_store(Request $request)
    {


        try {
            $input = $request->only('subject', 'project_id', 'description', 'priority', 'custom_field_1', 'custom_field_2', 'custom_field_3', 'custom_field_4', 'status', 'device_id');
            ...
            $input['test_on_issue_id'] = $request->input('search_product');
            $input['test_status'] = $request->input('test_status');
            ...
            $input['additional_value'] = $request->input('additional_value');
            ...
            // Create Test With task and workflow ID

            $projects = Project::where('id', $input['project_id'])->first();
            $method = SampleReading::where('business_id', $input['business_id'])
            ...
            // Generate UNique Test ID for tests
            if (!empty($method)) {
            ...
            // dd($method->test,$test_id);

            $tests = SampleAndTests::where('sample_id', $projects->product_id)->where('test_id', $input['test'])->get();

            // dd($tests);
            foreach ($tests as $test) {
                SampleReading::create([
                    'business_id' => $test->business_id,
                    'product_id' => $test->sample_id,
                    'test_group_id' => $test->test_id, // Adjust this based on your actual relationship structure
                    'test' => $test_id,
                    'workflow_id' => $projects->id,
                    'task_id' => $project_task->id,
                    'device_id' => $input['device_id'],
                    'group_id' => $test->group_id, // You need to define $groupId before this loop
                    'group_reading' => $test->group_reading, // You need to define $json before this loop
                    'value' => '0',
                    'status' => $request->input('status'), // Get the status from the input field
                ]);
            }

            AuditLogger::log('created', 'WF-Test', 'Test ID: ' . $test->id);
    ...
    public function postTaskStatus($id)
    {
        try {
            ...
            $project_task = ProjectTask::where('project_id', $project_id)
                ->findOrFail($id);
            $tests = SampleReading::where('workflow_id', $project_id)->where('task_id', $project_task->id)->get();
            //    dd(auth()->user()->id);

            foreach ($tests as $t) {
                $t->status = request()->input('status');
                $t->status_updated_by = auth()->user()->id;
                $t->save();
            }
            ...
            if ($status === 'completed') {
                $project_task->completed_on = now(); // Save current date and time
            }
            $project_task->save();
    ...
    public function test_status_update(Request $request)
    {

        // Update the task status
        $task = ProjectTask::where('id', $request->test_id)->update([
            'status' => $request->status,
        ]);
        $task = SampleReading::where('task_id', $request->test_id)->update([
            'status' => $request->status,
        ]);
