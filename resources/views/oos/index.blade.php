...
@section('content')
    ...
    <section class="content-header">
        <h1>@lang('lang_v1.oos')
            <small>@lang('lang_v1.manage_oos')</small>
        </h1>
        ...
        @component('components.widget', ['class' => 'box-primary'])
            <div class="tab-content">
                <div class="tab-pane active" id="">
                    @can('OOS.create')
                        <button type="button" class="btn btn-primary pull-right" data-toggle="modal" data-target="#addOOSModal">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active">
                                ...
                                <table class="table dataTable table-striped ajax_view hide-footer">
                                    <thead>
                                        <tr>
                                            <th style="padding: 10px; text-align: left;" scope="col">@lang('method.reported_at')</th>
                                            <th style="padding: 10px; text-align: left;" scope="col">@lang('method.oos_no')</th>
                                            <th style="padding: 10px; text-align: left;" scope="col">@lang('method.user') </th>
                                            <th style="padding: 10px; text-align: left;" scope="col">@lang('method.item_name')</th>
                                            <th style="padding: 10px; text-align: left;" scope="col">@lang('method.reason')</th>
                                            {{-- <th style="padding: 10px; text-align: left;" scope="col">Response</th> --}}
                                    ...
                                    <tbody>
                                        @foreach ($oos as $index => $oosItem)
                                            <tr>
                                                <td
                                                    style="padding: 10px; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px;">
                                                    {{ \Carbon\Carbon::parse($oosItem->created_at)->format('d-m-Y') }}
                                                </td>
                                                <td style="padding: 10px; text-align: left;">
                                                    {{ substr($oosItem->id, 0, 5) }}
                                                </td>
                                                <td
                                                    style="padding: 10px; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px;">
                                                    {{ @substr($oosItem->user->userFullName, 0, 15) }}
                                                </td>
                                                <td
                                                    style="padding: 10px; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px;">
                                                    {{ substr($oosItem->product_name, 0, 15) }}
                                                </td>
                                                <td
                                                    style="padding: 10px; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px;">
                                                    {{ substr(strip_tags($oosItem->reason), 0, 15) }}
                                                </td>
                                                {{-- <td
                                                style="padding: 10px; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px;">
                                                {{ substr($oosItem->response, 0, 15) }}
                                            </td> --}}
                                                <td style="padding: 10px; text-align: left;">
                                                    <div class="dropdown">
                                                        <button class="btn btn-primary btn-xs dropdown-toggle" type="button"
                                                            id="actionMenu{{ $oosItem->id }}" data-toggle="dropdown"
                                                            aria-haspopup="true" aria-expanded="false">
                                                            @lang('lang_v1.actions')
                                                            <span class="caret"></span>
                                                        </button>
                                                        <div class="dropdown-menu"
                                                            aria-labelledby="actionMenu{{ $oosItem->id }}">
                                                            @can('OOS.view')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('oos.show', $oosItem->id) }}">
                                                                    <i class="fas fa-eye"></i> @lang('messages.view')
                                                                </a>
                                                            @endcan
                                                            @can('OOS.edit')
                                                                <a class="dropdown-item edit-oos-btn" href="#"
                                                                    data-toggle="modal"
                                                                    data-target="#editOosModal{{ $index }}"
                                                                    data-oos-id="{{ $oosItem->id }}">
                                                                    <i class="fas fa-edit"></i> @lang('messages.edit')
                                                                </a>
                                                            @endcan
                                                            @can('OOS.delete')
                                                                <form method="POST"
                                                                    action="{{ route('oos.destroy', $oosItem->id) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit" class="dropdown-item delete-oos">
                                                            ...
                                                            @can('logs.view')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('logs.index', ['module' => 'oos']) }}">
...
@section('javascript')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.delete-oos', function(e) {
                e.preventDefault();
