...
<script>
    var lastNotificationCount = {{ auth()->check() ? auth()->user()->unreadNotifications()->count() : 0 }};
    ...
    setInterval(function() {
        $.ajax({
            ...
            success: function(response) {
                var count = response.total_unread;
                if (count != lastNotificationCount) {
                    lastNotificationCount = count;
                    // Notification count changed, update the UI
                    if (count > 0) {
...
<script>
    $(document).on('click', '.load_notifications', function() {
        $.ajax({
