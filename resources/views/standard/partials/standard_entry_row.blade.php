@foreach ($variations as $variation)
    <tr @if (!empty($purchase_order_line)) data-purchase_order_id="{{ $purchase_order_line->transaction_id }}" @endif
        ...
        <td>
            ...
            @if (!empty($sub_units))
                <br>
                <select style="display: none" name="purchases[{{ $row_count }}][sub_unit_id]" class="form-control input-sm sub_unit">
        ...
        <td>
            <div class="input-group">
                {!! Form::select(
                    'purchases[' . $row_count . '][batch_no]',
                    $batch_no,
                    !empty($duplicate_product->batch_no) ? $duplicate_product->batch_no : null,
                    ['placeholder' => __('messages.please_select'), 'class' => 'form-control' , 'id' => 'batch_no'],
                ) !!}
                <span class="input-group-btn">
                    <button type="button" class="btn btn-default bg-white btn-flat btn-modal"
                        data-href="{{ action([\App\Http\Controllers\BatchController::class, 'create'], ['quick_add' => true, 'product_id' => $product->id]) }}"
        ...
        {{-- @if (empty($is_purchase_order)) --}}
            ...
            <td style="display: none">
                <div class="input-group">
                    {!! Form::select(
                        'purchases[' . $row_count . '][contract_no]',
                        $contract_no,
                        !empty($duplicate_product->contract_no) ? $duplicate_product->contract_no : null,
                        ['placeholder' => __('messages.please_select'), 'class' => 'form-control  ','id'=>'contract_no'],
                    ) !!}
                    <span class="input-group-btn">
                        <button type="button" class="btn btn-default bg-white btn-flat btn-modal"
                            data-href="{{ action([\App\Http\Controllers\ContractController::class, 'create'], ['quick_add_contract' => true]) }}"
                            title="@lang('brand.add_brand')" data-container=".view_modal"><i
                                class="fa fa-plus-circle text-primary fa-lg"></i></button>
                    </span>
                </div>
            ...
            <td style="display: none">
                <div class="form-group">
                    {{-- {!! Form::label('instalments', __('product.instalments') . ':*') !!}  --}}
                    @php
                        $staticOptions_dosage_form = [
                            '' => 'Please Select',
                            'instalments_1' => 'Installments 1',
                            'instalments_2' => 'Installments 2',
                            'instalments_3' => 'Installments 3',
                            'instalments_4' => 'Installments 4',
                            'instalments_5' => 'Installments 5',
                           
                        ];
                        $d_types = $staticOptions_dosage_form;
                    @endphp
                    {!! Form::select('purchases[' . $row_count . '][instalments]', $d_types, !empty($duplicate_item->type) ? $duplicate_item->type : null, [
                        'class' => 'form-control',
                        'required',
            ...
            <td>
                ...
                <div class="form-group">
                    @php
                      ...
                      $samples = \App\Product::forSampleNameSearchDropdown($business_id);
                    @endphp
                   <select name="purchases[{{ $row_count }}][associated_sample]" class="form-control select2" required>
                    <option value="" selected disabled>{{ __('messages.please_select') }}</option>
                    @foreach ($samples as $key => $sample)
                        <option value="{{ $key }}" {{ !empty($duplicate_item->type) && $duplicate_item->type == $key ? 'selected' : '' }}>
                            {{ $sample }}
                        </option>
                    @endforeach
                </select>
