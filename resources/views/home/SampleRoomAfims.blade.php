...
@section('content')
    ...
    <section class="content content-custom no-print">
        <!--Sample Room AFMSL Dashboard -->
        @if (auth()->check() &&
                auth()->user()->hasRole('SampleRoom' . '#' . $business_id))


            <!-- Forwarded Samples Data -->
            @component('components.dashbord_widget', ['class' => 'box-primary', 'title' => __('home.sample_d_fwd')])
                <div class="row main-contain" style="padding: 20px; margin-top: -20px;">
                    <div class="col-sm-4 col-custom">
                        <div class="info-box info-box-new-style bg-blue"
                            style="padding: 20px; border-radius: 10px; display: flex; align-items: center;">
                            <div style="flex: 1; display: flex; justify-content: center; align-items: center;">
                                <span class="info-box-icon" style="font-size: 2em;"><i class="fas fa-share-square"></i></span>
                            </div>
                            <div class="info-box-content"
                                style="flex: 3; display: flex; flex-direction: column; justify-content: center; padding: 20px; margin-left: 30px;">
                                <div
                    ...
                    <div class="chart-container col-sm-8 col-custom">
                        <div id="forwardedSamplesBarChart" style="height: 400px;"></div>
                    </div>
                </div>
            @endcomponent
        @endif
...
@section('javascript')
    ...
    <script>
        $(document).ready(function() {
            $.ajax({
                url: "{{ route('sampleRoomAfims.dashboard') }}",
                method: 'get',
                data: {
                    _token: "{{ csrf_token() }}"
                },
                success: function(response) {
                    if (response.success == true) {
                        ...
                        var forwardedOptions = {
                            ...
                            title: {
                                text: 'Samples Forwarded To 2IC Overview',
                                align: 'left',
                                style: {
                                    fontSize: '16px',
                                    fontWeight: 'bold'
