@extends('layouts.app')
@section('title', __('lang_v1.spillage_unforeseen'))

@section('content')
    <!-- Content Header (Page header) -->

    <section class="content-header">
        <h1>@lang('lang_v1.spillage_unforeseen')
            <small>@lang('lang_v1.manage_spillage_unforeseen')</small>
        </h1>
        ...
        @component('components.widget', ['class' => 'box-primary'])
            <div class="tab-content">
                <div class="tab-pane active" id="">
                    @can('spillage.create')
                        <a class="btn btn-primary pull-right btn-modal" data-href="{{ route('spillages.create') }}"
                            data-container=".addSpillageModal">
                    ...
                    <div class="nav-tabs-custom">
                        <div class="tab-content">
                            <div class="tab-pane active">
                                <table id="spillagesTable" class="table dataTable table-striped ajax_view hide-footer">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>@lang('method.remarks')</th>
                                            <th>@lang('method.date_time')</th>
                                            <th>@lang('method.reported_by')</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($spillages as $spillage)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $spillage->chemical->name }}</td>
                                                {{-- <td>{{ $spillage->standard->name }}</td> --}}
                                                <td>{{ $spillage->spillage_quantity }}</td>
                                                <td>{!! $spillage->spillage_remarks !!}</td>
                                                <td>{{ \Carbon\Carbon::parse($spillage->spillage_date_time)->format('d M Y H:i ') }}
                                                </td>
                                                <td>{{ $spillage->user->getUserFullNameAttribute() }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
            ...
            <div class="modal fade addSpillageModal" id="addSpillageModal" tabindex="-1" role="dialog"
                aria-labelledby="addSpillageModalLabel" aria-hidden="true">
            </div>
        @endcomponent
    </section>
    <style>
...
@section('javascript')
    ...
    `);
            ...
            // $(document).on('click', '.delete-spillage', function(e) {
            //     e.preventDefault();
            ...
            function logPrintEvent() {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        printedModule: 'Spillage/Unforeseen'
                    },
