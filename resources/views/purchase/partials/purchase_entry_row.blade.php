@foreach ($variations as $variation)
    <tr @if (!empty($purchase_order_line)) data-purchase_order_id="{{ $purchase_order_line->transaction_id }}" @endif
        ...
        <td>
            ({{ $product->pv_number }}) {{ $product->name }} ({{ $variation->sub_sku }})
        ...
        <td>
            ...
            @if (!empty($sub_units))
                <br>
                <select style="display: none" name="purchases[{{ $row_count }}][sub_unit_id]"
            ...
            <div class="input-group">
                ...
                <span class="input-group-btn">
                    <button type="button" class="btn btn-default bg-white btn-flat btn-modal"
                        data-href="{{ action([\App\Http\Controllers\BatchController::class, 'create'], ['quick_add' => true, 'product_id' => $product_id??null]) }}"
        ...
        <td class="{{ $hide_tax }}" style="display: none">
            @php
                ...
                $dpp_inc_tax = !empty($purchase_order_line)
                    ? number_format(
                        $purchase_order_line->purchase_price_inc_tax / $purchase_order->exchange_rate,
                        $currency_precision,
                        $currency_details->decimal_separator,
                        $currency_details->thousand_separator,
                    )
        ...
        <td>
            <div class="input-group">
                ...
                <span class="input-group-btn">
                    <button type="button" class="btn btn-default bg-white btn-flat btn-modal"
                        data-href="{{ action([\App\Http\Controllers\QuickAddContractController::class, 'create'], ['quick_add_contract' => true, 'product_id' => $product_id ?? null]) }}"
                        title="@lang('brand.add_brand')" data-container=".view_modal"><i
                            class="fa fa-plus-circle text-primary fa-lg"></i></button>
                </span>
            </div>
            ...
            <div class="form-group">
                ...
                @php
                    $staticOptions_dosage_form = [
                        ...
                        // 'instalments_5' => 'Instalments 5',
                    ];
                    $d_types = $staticOptions_dosage_form;
                @endphp
                {!! Form::select(
                    'purchases[' . $row_count . '][instalments]',
                    $d_types,
