...
<tr class="product_row" data-row_index="{{ $row_count }}"
    ...
    <td>
        ...
        <input type="hidden" name="products[{{ $row_count }}][product_sku]" class="form-control "
    ...
    {{-- Sample ISSued Id for Reagent and Standard --}}
    <td>
        <div class="form-group">
            <div class="input-group">
                {!! Form::select('issued_id', $invoiceNumbers, !empty($duplicate_product->invoiceNumbers) ? $duplicate_product->invoiceNumbers : null, ['placeholder' => __('messages.please_select'), 'class' => 'form-control ', 'id' => 'issued_id']) !!}
            </div>
            <div class="" >
                <div class="">
                    <h6>Sample: <span id="sample_name"></span> </h6>
                    <h6>Batch NO:  <span id="batch_no"></span> </h6>
                </div>
            </div>
        </div>
    </td>

         {{-- Standard Coustome TYpe for use in db  --}}
        ...
        <div class="form-group">
            <div class="input-group">
                ...
                {!! Form::select(
                    'standard_type',
                    $standrd_type,
                    !empty($duplicate_product->standrd_type) ? $duplicate_product->standrd_type : null,
...
<script>
    $(document).ready(function () {
        ...
        $('#issued_id').on('change', function () {
            // Get the selected value
            var selectedValue = $(this).val();

            // Make an AJAX request to fetch details based on the selected value
            $.ajax({
                url: '/standard/issue_record/sample/details/' + selectedValue,
                type: 'GET',
                dataType: 'json',
                success: function (data) {
                    var sampleName = data.product.name;
                    var batchCode = data.batch.code;
                    console.log(sampleName,batchCode,data);
                    // Handle the response and update your UI with the details
                    $('#sample_name').html(sampleName);
                    $('#batch_no').html(batchCode);
                    // Update your UI here based on the data received
                },
