@extends('layouts.app')
@section('title', __('lang_v1.delivery_persons'))

@section('content')

    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>@lang('lang_v1.delivery_persons')
            <small>@lang('lang_v1.m_delivery_persons')</small>
        </h1>
    </section>
    <!-- Main content -->
        ...
        @component('components.widget', ['class' => 'box-primary'])
            <div class="tab-content">
                <div class="tab-pane active" id="">
                    @can('delivery_person.create')
                        <button type="button" class="btn btn-primary pull-right btn-modal"
                            data-href="{{ route('delivery_persons.create') }}" data-container=".view_modal">
                            ...
                            <div class="tab-pane active">
                                <table class="table dataTable table-striped ajax_view hide-footer">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th class="no-print">@lang('messages.picture')</th>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($deliveryPerson->picture)
                                                        <a href="{{ route('delivery_persons.show', $deliveryPerson->id) }}">
                                                            <img style="border-radius:10px; width:70px; object-fit:contain;"
                                                                src="{{ asset('uploads/' . $deliveryPerson->picture) }}">
                                                        </a>
                                                    @else
                                                        <a href="{{ route('delivery_persons.show', $deliveryPerson->id) }}">
                                                            <img style="border-radius:50%; width:70px; object-fit:contain;"
                                                ...
                                                <td style="padding: 10px; text-align: left;">
                                                    <div class="dropdown">
                                                        <button class="btn btn-primary btn-xs dropdown-toggle" type="button"
                                                            id="actionMenu{{ $deliveryPerson->id }}" data-toggle="dropdown"
                                                            aria-haspopup="true" aria-expanded="false">
                                                            Actions <span class="caret"></span>
                                                        </button>
                                                        <div class="dropdown-menu"
                                                            aria-labelledby="actionMenu{{ $deliveryPerson->id }}">
                                                            @can('delivery_person.view')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('delivery_persons.show', $deliveryPerson->id) }}">
                                                                    <i class="fas fa-eye"></i> @lang('messages.view')
                                                                </a>
                                                            @endcan
                                                            @can('delivery_person.edit')
                                                                <a class="dropdown-item"
                                                                    href="{{ route('delivery_persons.edit', $deliveryPerson->id) }}">
                                                                    <i class="fas fa-edit"></i> @lang('messages.edit')
                                                                </a>
                                                            @endcan

                                                            @can('delivery_person.delete')
                                                                <form method="POST"
                                                                    action="{{ route('delivery_persons.destroy', $deliveryPerson->id) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
                                                                        class="dropdown-item delete-delivery-person">
                                                                        <i class="fas fa-trash"></i> @lang('messages.delete')
                                                                    </button>
                                                                </form>
                                                            @endcan <a class="dropdown-item"
                                                                href="{{ route('logs.index', ['module' => 'Delivery Person']) }}">
...
@section('javascript')
    ...
    <script>
        ...
        `);
            ...
            $(document).on('click', '.delete-delivery-person', function(e) {
                e.preventDefault();
