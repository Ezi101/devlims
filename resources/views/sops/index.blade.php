@extends('layouts.app')
@section('title', __('lang_v1.sops'))

@section('content')

    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>@lang('lang_v1.sops')
            <small>@lang('lang_v1.manage_sops')</small>
        </h1>
    </section>
                        ...
                        <div class="col-md-4">
                            <div class="filter-wrapper">
                                ...
                                <select id="categories-filter" class="form-control select2" style="width: 100%;">
                                    ...
                                    @foreach ($sops as $sop)
                                        <option value="{{ $sop->category }}">{{ $sop->category }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
        ...
        @component('components.widget', ['class' => 'box-primary'])
            <div class="tab-content">
                <div class="tab-pane active">
                    @can('SOPs.create')
                        <a class="btn btn-primary pull-right btn-modal" data-href="{{ route('sops.create') }}"
                            data-container=".addSopModal">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active">
                                <table class="table dataTable table-striped ajax_view hide-footer">
                                    <thead>
                                        <tr>
                                            <th style="display: none;">@lang('method.hash_sign')</th>
                                            <th>@lang('method.date')</th>
                                            <th>@lang('method.reference_no')</th>
                                            <th>@lang('method.title')</th>
                                            <th>@lang('method.category')</th>
                                            <th>@lang('method.expiry')</th>
                                            {{-- <th
                                    ...
                                    <tbody>
                                        @foreach ($sops as $sop)
                                            <tr>
                                                ...
                                                <td>
                                                    <a
                                                        href="{{ route('sops.show', $sop->id) }}">{{ substr($sop->reference_code, 0, 12) }}</a>
                                                </td>
                                                <td>{{ $sop->title }}</td>
                                                <td>{{ $sop->category }}</td>
                                                <td>{{ $sop->sop_expiry_date }}</td>
                                                {{-- <td
                                                    style="padding: 10px; text-align: left; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px;">
                                                    {!! substr(strip_tags($sop->description), 0, 100) !!}
                                                </td> --}}

                                                <td>
                                                    <div class="dropdown">
                                                        <button class="btn btn-primary btn-xs dropdown-toggle" type="button"
                                                            id="actionMenu{{ $sop->id }}" data-toggle="dropdown"
                                                            aria-haspopup="true" aria-expanded="false">
                                                            @lang('lang_v1.actions') <span class="caret"></span>
                                                        </button>
                                                        <div class="dropdown-menu"
                                                            aria-labelledby="actionMenu{{ $sop->id }}">
                                                            <a class="dropdown-item" href="{{ route('sops.show', $sop->id) }}">
                                                                <i class="fas fa-eye"></i> @lang('messages.view')
                                                            </a>
                                                            @can('SOPs.edit')
                                                                <a class="dropdown-item" href="{{ route('sops.edit', $sop->id) }}">
                                                                    <i class="fas fa-edit"></i> @lang('messages.edit')
                                                                </a>
                                                            @endcan

                                                            @can('SOPs.delete')
                                                                <form action="{{ route('sops.destroy', $sop->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit" class="dropdown-item delete-sop">
                                                                        <i class="fas fa-trash"></i> @lang('messages.delete')
                                                                    </button>
                                                                </form>
                                                            @endcan
                                                            <a class="dropdown-item"
                                                                href="{{ route('logs.index', ['module' => 'sop']) }}">
            ...
            <div class="modal fade addSopModal" id="addSopModal" tabindex="-1" role="dialog"
                aria-labelledby="addSopModalLabel" aria-hidden="true">
            </div>
        @endcomponent
    </section>
    <style>
...
@section('javascript')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#categories-filter').on('change', function() {
