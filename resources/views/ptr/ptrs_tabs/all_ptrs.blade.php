                                    ...
                                    <tr data-url='{{ url('/samples/pre/test/report/view/' . $ptr->ptr_no) }}'
                                        ...
                                        @php
                                            if (Auth::user()->hasRole('Quality Assurance#' . $business_id)) {
                                                $app_ptr = App\PTR_STR_Approval::where('remark_by', Auth::user()->id)
                                                    ...
                                                    @if (auth()->user()->hasRole('Quality Assurance' . '#' . $business_id))
                                                        @if (auth()->user()->can('str.approve_with_remarks'))
                                                            <a class="dropdown-item btn btn-modal"
                                                                ...
                                                                <i class="fa fa-message"></i> @lang('APPROVAL')
                                                            </a>
                                                        @endif
                                                    @endif
                                                    @if (auth()->user()->hasRole('OC' . '#' . $business_id))
                                                        @if (auth()->user()->can('str.approve_with_remarks') && !empty($app_ptr))
                                                            <a class="dropdown-item btn btn-modal"
                                                                data-href="{{ route('ptr_approval', ['ptr_no' => $ptr->ptr_no]) }}"
                                                                data-container=".ptr_approval">
                                                                <i class="fa fa-message"></i> @lang('APPROVAL')
...
@section('javascript')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const statusCell = document.querySelectorAll('#active');
            statusCell.forEach(cell => {
                cell.addEventListener('click', function(event) {
                    ...
                    if (newStatus === 'active') {
                        this.querySelector('.label').classList.remove('bg-red');
                        this.querySelector('.label').classList.add('bg-green');
                        this.querySelector('.label').textContent = 'Active';
                    } else {
                        this.querySelector('.label').classList.remove('bg-green');
                        this.querySelector('.label').classList.add('bg-red');
                        this.querySelector('.label').textContent = 'Inactive';
                    }

                    var ptrId = this.dataset.ptr_id;
