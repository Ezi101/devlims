...
$(document).ready(function () {
    ...
    $('select#customer_id').select2({
        ...
        templateResult: function (data) {
            ...
            var template = '';
            if (data.username) {
                template += data.username + '<br>';
            }
            // console.log(data);
            template += data.text + '<br>' + data.role + '<br>' + data.email;
    ...
    $('#customer_id').on('select2:select', function (e) {
        ...
        if (data.pay_term_type) {
            ...
            $('#add_sell_form select[name="pay_term_type"]').val('days');
            $('#edit_sell_form select[name="pay_term_type"]').val('days');
        }
        ...
        $('#search_product')
            .autocomplete({
                ...
                focus: function (event, ui) {
                    console.log(ui.item.qty_available);
                ...
                select: function (event, ui) {
                    ...
                    if (
                        ...
                        $('#information_div_id').html(`
                            <div class="product-info">
                                <h4> ${ui.item.name} <span>- Available Qty (${Math.floor(
                                    ui.item.qty_available
                                )})</span></h4>
                            </div>
                        `);
                    } else {
    ...
    $('button#add-payment-row').click(function () {
        ...
        $.ajax({
            method: 'POST',
            url: '/issue-stock/pos/get_payment_row',
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.success == 1) {
                            ...
                            // Handle batch quantity info if present
                            console.log('=== ERROR RESPONSE ===');
                            console.log('Full result object:', result);
                            console.log('result.batch_qty_info:', result.batch_qty_info);
                            console.log('Type of batch_qty_info:', typeof result.batch_qty_info);
                            console.log('Is array?:', Array.isArray(result.batch_qty_info));
                            if (result.batch_qty_info) {
                                console.log('batch_qty_info length:', result.batch_qty_info.length);
                            }
                            console.log('result.msg:', result.msg);
                            console.log('=== END RESPONSE ===');

                            if (
                                result.batch_qty_info &&
                                Array.isArray(result.batch_qty_info) &&
                                result.batch_qty_info.length > 0
                            ) {
                                // Create a custom modal popup with batch details
                                var modalHtml = `
                                    <div class="modal fade" id="batchErrorModal" tabindex="-1" role="dialog" aria-labelledby="batchErrorModalLabel">
                                        <div class="modal-dialog modal-lg" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header bg-danger text-white">
                                                    <h5 class="modal-title" id="batchErrorModalLabel">
                                                        <i class="fa fa-exclamation-triangle"></i> Cannot Issue Sample
                                                    </h5>
                                                ...
                                                <div class="modal-body">
                                                    <p class="text-danger mb-3"><strong>One or more batches have insufficient quantity:</strong></p>
                                                    <div class="batch-error-list">
                                `;

                                // Group by lab
                                var groupedByLab = {};
                                result.batch_qty_info.forEach(function (batch) {
                                    if (!groupedByLab[batch.lab]) {
                                        groupedByLab[batch.lab] = [];
                                    }
                                    groupedByLab[batch.lab].push(batch);
                                });

                                // Build modal content
                                Object.keys(groupedByLab).forEach(function (lab) {
                                    modalHtml += `<h6 class="mt-3 mb-2"><strong>${lab} Lab:</strong></h6>`;
                                    modalHtml += `<table class="table table-sm table-bordered">
                                    ...
                                    groupedByLab[lab].forEach(function (batch) {
                                        var rowClass = '';
                                        if (batch.is_zero || batch.is_insufficient) {
                                            rowClass = 'class="table-danger"';
                                        }
                                        modalHtml += `<tr ${rowClass}>
                                            <td><strong>${batch.batch_name}</strong></td>
                                            <td>${batch.quantity}</td>
                                            <td>${batch.required_qty}</td>
                                        </tr>`;
                                    });
                                ...
                                $('#batchErrorModal').remove();

                                // Add and show modal
                                $('body').append(modalHtml);
                                $('#batchErrorModal').modal('show');

                                // Clean up when modal closes
                                $('#batchErrorModal').on('hidden.bs.modal', function () {
                                    $(this).remove();
                                });

                                console.log('✓ Batch error modal shown');
                            } else {
                                console.log('✗ batch_qty_info not valid, showing toastr instead');
                                toastr.error(result.msg);
                            }
                        }
    ...
    $('form#quick_add_contact')
        ...
        .validate({
            ...
            submitHandler: function (form) {
                console.log('quick add contact form submit');
                $.ajax({
                    method: 'POST',
    ...
    function updateTotalQty() {
        var total_qty = 0;
        $('#batch_ids_div').html('');
        $('#searched_product_name').html('');

        $('.batches-qty-input').each(function () {
            var input = $(this);
            var available_qty_input = input
                .parent()
                .parent()
                .find('.available-qty-input-' + window.first_words);
            var available_qty =
                available_qty_input.length > 0 ? parseFloat(available_qty_input.val()) : 0;
            var product_id = parseInt(
                input
                    .parent()
                    .parent()
                    .find('.product_id-' + window.first_words)
                    .val()
            );
            var product_name = input
                .parent()
                .parent()
                .find('.' + window.first_words)
                .val();
            let id_to_put_the_name = '#searched_product_name-' + window.first_words;

            $(id_to_put_the_name).html(product_name);
            var batch_id = parseInt(
                input
                    .parent()
                    .parent()
                    .find('.batch_id-' + window.first_words)
                    .val()
            );
            var val = parseFloat(input.val());
            console.log(val, available_qty);
            if (!isNaN(val) && val <= available_qty) {
                total_qty += val;
                console.log(total_qty);
            } else {
                input.val(available_qty);
                toastr.error("Quantity can't be more than available quantity");
                return false;
            }
            console.log(val);
            if (batch_id) {
                let div_id = '#batch_ids_div-' + window.first_words;
                $(div_id).append(
                ...
                $(div_id).append(
                    '<input type="hidden" name="batch_wise_qty-' +
                        window.first_words +
                        '[]" value="' +
                        val +
                        '">'
        ...
        let total_qty_id = '#total_qty-' + window.first_words;
        $(total_qty_id).val(total_qty);
    }

    $(document).on('input', '.batches-qty-input', function () {
        updateTotalQty();
    });
    ...
    $('button#submit-sell, button#save-and-print, button#save_and_print_labels').click(function (e) {
        var buttonId = $(this).attr('id');

        // For save-and-print and save_and_print_labels, do validation first
        if (buttonId === 'save-and-print' || buttonId === 'save_and_print_labels') {
            //Check if product is present or not.
            var is_msp_valid = true;
            //Validate minimum selling price if hidden
            $('.pos_unit_price_inc_tax').each(function () {
            ...
            if (buttonId == 'save-and-print') {
                $('#is_save_and_print').val(1);
            } else {
                $('#is_save_and_print').val(0);
            }

            if (buttonId == 'save_and_print_labels') {
                $('#is_save_and_print_labels').val(1);
            } else {
                $('#is_save_and_print_labels').val(0);
            }
        ...
        // Get form and submit via AJAX
        var batchForm =
            $('#batch_sample_form').length > 0 ? $('#batch_sample_form') : $('#add_sell_form');
        var formData = batchForm.serialize();
        var formAction = batchForm.attr('action');
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == 1) {
                    ...
                    var seenBatches = {};
                    result.batch_qty_info.forEach(function (batch) {
                        if (!seenBatches[batch.batch_name]) {
                            batchList +=
                                batch.batch_name +
                                ' (Qty: ' +
                                batch.quantity +
                                ...
                                batch.required_qty +
                                ')\n';
                            seenBatches[batch.batch_name] = true;
                        }
                    });
                    toastr.error('Cannot Issue Sample\n\n' + batchList, 'Insufficient Quantity');
                } else {
                    toastr.error(result.msg);
                }
            },
    ...
    // Old code - keeping for reference, can be removed
    /* $('button#save-and-print , button#save_and_print_labels').click(function (e) {
        //Check if product is present or not.
        ...
        if ($(this).attr('id') == 'save_and_print_labels') {
            $('#is_save_and_print_labels').val(1);
        } else {
            $('#is_save_and_print_labels').val(0);
        }
...
function get_featured_products() {
    ...
    if (location_id && $('#featured_products_box').length > 0) {
        $.ajax({
            method: 'GET',
            url: '/issue/stock/pos/get-featured-samples/' + location_id,
            dataType: 'html',
            success: function (result) {
                if (result) {
...
function get_product_suggestion_list(
    ...
    if (url == null) {
        url = '/issue-stock/pos/get-sample-suggestion';
    }
...
function get_recent_transactions(status, element_obj) {
    ...
    $.ajax({
        method: 'GET',
        url: '/issue/stock/pos/get-recent-transactions',
...
function pos_product_row(
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            method: 'GET',
            url: '/issue-stock/pos/get_sample_row/' + variation_id + '/' + location_id,
            async: false,
            data: {
            ...
            success: function (result) {
                console.log(result);
                if (result.success) {
                    $('#pos_table tbody').empty().append(result.html_content);
                    $('body').append(result.modal_content); // Ensure modals are appended to the body
                    reinitializeModals(); // Reinitialize the modals
                    updateTotalQty();
                    // increment row count
...
function getCustomerRewardPoints() {
    ...
    $.ajax({
        method: 'POST',
        url: '/issue/stock/pos/get-reward-details',
        data: {
            customer_id: customer_id,
        },
        dataType: 'json',
...
$(document).on('change', '#types_of_service_id', function () {
    ...
    if (types_of_service_id) {
        $.ajax({
            method: 'POST',
            url: '/issue/stock/get-types-of-service-details',
            data: {
