$(document).ready(function () {
    ...
    if ($('#sr_date_filter').length == 1) {
        ...
        if ($('#sr_payments_with_commission_table').length > 0) {
            sr_payments_with_commission_report = $(
                ...
                aaSorting: [[1, 'desc']],
                ajax: {
                    url: '/reports/issue-stock-payment-report',
                    data: function (d) {
    ...
    product_purchase_report = $('table#product_purchase_report_table').DataTable({
        ...
        aaSorting: [[3, 'desc']],
        ajax: {
            url: '/reports/sample-receive-stock-report',
            data: function (d) {
                var start = '';
                var end = '';
    ...
    if ($('#search_product').length > 0) {
        $('#search_product').autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: '/receive-stock/get_samples?check_enable_stock=false',
                    dataType: 'json',
                    data: {
                        term: request.term,
                    },
    ...
    if ($('table#product_sell_report_table').length == 1) {
        ...
        product_sell_report = $('table#product_sell_report_table').DataTable({
            ...
            aaSorting: [[6, 'desc']],
            ajax: {
                url: '/reports/sample-issue-report',
                data: function (d) {
                    var start = '';
                    var end = '';
    ...
    product_sell_report_with_purchase_table = $(
        ...
        aaSorting: [[4, 'desc']],
        ajax: {
            url: '/reports/sample-issue-report-with-purchase',
            data: function (d) {
                var start = '';
                var end = '';
                var start_time = $('#product_sr_start_time').val();
    ...
    product_sell_grouped_report = $('table#product_sell_grouped_report_table').DataTable({
        ...
        aaSorting: [[1, 'desc']],
        ajax: {
            url: '/reports/sample-issue-grouped-report',
            data: function (d) {
                var start = '';
                var end = '';
                var start_time = $('#product_sr_start_time').val();
    ...
    purchase_payment_report = $('table#purchase_payment_report_table').DataTable({
        ...
        aaSorting: [[2, 'desc']],
        ajax: {
            url: '/reports/receive-stock-payment-report',
            data: function (d) {
                d.supplier_id = $('select#supplier_id').val();
                d.location_id = $('select#location_id').val();
                var start = '';
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        scrollCollapse: true,
        ajax: {
            url: '/search/supplier/report',
            data: function (d) {
                if ($('#purchase_list_filter_supplier_id').length) {
                    d.supplier_id = $('#purchase_list_filter_supplier_id').val();
                }
                if ($('#purchase_list_filter_report_id').length) {
                    var reportId = $('#purchase_list_filter_report_id').val();
                    if (reportId !== null) {
                        var parts = reportId.split('(');
                        d.report_id = parts[0];
                    } else {
                        console.error("Value of $('#purchase_list_filter_report_id') is null.");
                    }
                }
                if ($('#purchase_list_filter_contract_type').length) {
                    d.contract_type = $('#purchase_list_filter_contract_type').val();
                }
                if ($('#purchase_list_filter_contract_no').length) {
                    d.contract_no = $('#purchase_list_filter_contract_no').val();
                }
                if ($('#purchase_list_filter_instalments').length) {
                    d.instalment = $('#purchase_list_filter_instalments').val();
                }
        ...
        columns: [
            ...
            // {
            //     data: 'sample_and_generic_name',
            //     name: 'sample_and_generic_name',
            // },
            {
                data: 'sample_name',
                name: 'sample_name',
    ...
    $(document).on(
        'change',
        '#purchase_list_filter_report_id,  #purchase_list_filter_supplier_id,  #purchase_list_filter_contract_type,  #purchase_list_filter_contract_no,  #purchase_list_filter_instalments',
        function () {
            purchase_table.ajax.reload();
        }
    );
    $(document).on('change', '.purchase_list_filter_contract_no', function () {
        var contract_no_ins = $('.purchase_list_filter_contract_no').val();
        var type = $('.purchase_list_filter_contract_type').val();

        if (contract_no_ins && type != 'tender' && type != 'other') {
            $.ajax({
                url: '/search/istalments/' + contract_no_ins,
                type: 'GET',
                success: function (response) {
                    console.log(response);
                    $('#purchase_list_filter_instalments').empty();
                    $('#purchase_list_filter_instalments').append(
                        $('<option>', {
                            value: '',
                            text: 'All',
                        })
                    ...
                    $.each(response, function (index, purchase) {
                        $('.purchase_list_filter_instalments').append(
                            $('<option>', {
                                value: purchase.instalments,
                                text: purchase.instalments,
                            })
                        );
                    });
                },
    ...
    $(document).on('change', '#purchase_list_filter_contract_type', function () {
        var type = $('.purchase_list_filter_contract_type').val();
        if (type == 'tender') {
            $("label[for='purchase_list_filter_contract_no']").text('Tender No');
            $('.inst_hide').hide();
            $('#purchase_list_filter_instalments').empty();
            $('#purchase_list_filter_instalments').append(
                $('<option>', {
        ...
        if (type == 'other') {
            $("label[for='purchase_list_filter_contract_no']").text('Letter No');
            $('.inst_hide').hide();
            $('#purchase_list_filter_instalments').empty();
            $('#purchase_list_filter_instalments').append(
                $('<option>', {
        ...
        if (type == 'supply') {
            $('.inst_hide').show();
            $("label[for='purchase_list_filter_contract_no']").text('Contract No');
        }
        if (type) {
            $.ajax({
                url: '/search/contract/no/' + type,
                type: 'GET',
                success: function (response) {
                    console.log(response);
                    $('#purchase_list_filter_contract_no').empty();
                    $('#purchase_list_filter_contract_no').append(
                        $('<option>', {
                            value: '',
                            text: 'All',
                        })
    ...
    $(document).on('change', '.purchase_list_filter_contract_type', function () {
        var columnIndex = purchase_table.column('instalments:name').index();
        var type = $('.purchase_list_filter_contract_type').val();

        if (type == 'other' || type == 'tender') {
            purchase_table.column(columnIndex).visible(false);
        } else {
            purchase_table.column(columnIndex).visible(true);
        }
    });
    $(document).on('change', '#purchase_list_filter_supplier_id', function () {
        var supplier_id = $('#purchase_list_filter_supplier_id').val();
        if (supplier_id) {
            $.ajax({
                url: '/search/supplier/report/samples/' + supplier_id,
                type: 'GET',
                success: function (response) {
                    console.log(response);
                    $('#purchase_list_filter_report_id').empty();
                    $('#purchase_list_filter_report_id').append(
                        $('<option>', {
                            value: '',
                            text: 'All',
                        })
                    ...
                    $.each(response, function (index, purchase) {
                        $('#purchase_list_filter_report_id').append(
                            $('<option>', {
                                value: purchase.sample_and_generic_name,
                                text: purchase.sample_and_generic_name,
                            })
                        );
                    });
                },
...
function updatePurchaseSell() {
    ...
    $.ajax({
        method: 'GET',
        url: '/reports/receive-stock-issue-stock',
        dataType: 'json',
        data: data,
        success: function (data) {
            $('.total_purchase').html(
