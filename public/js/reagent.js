$(document).ready(function () {
    ...
    //Search Sample TO DEMAND STOCK
    if ($('#demand_product').length > 0) {
        $('#demand_product')
            .autocomplete({
                source: function (request, response) {
                    $.getJSON(
                        '/demand-stock/get_samples',
                        {
                ...
                select: function (event, ui) {
                    $(this).val(null);
                    get_demand_entry_row(ui.item.product_id, ui.item.variation_id);
                },
            })
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        scrollCollapse: true,
        ajax: {
            url: '/reagent/recevied-stock/index',
            data: function (d) {
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        $.ajax({
            method: 'POST',
            url: '/receive-stock/get_ragent-_stock_entry_row',
            dataType: 'html',
            data: data,
            success: function (result) {
...
function get_demand_entry_row(product_id, variation_id) {
    if (product_id) {
        var row_count = $('#row_count').val();
        var location_id = $('#location_id').val();
        var supplier_id = $('#supplier_id').val();
        ...
        $.ajax({
            method: 'get',
            url: '/demand-stock/get_sample_demand-_stock_entry_row',
            dataType: 'html',
            data: data,
            success: function (result) {
                append_purchase_lines(result, row_count);
