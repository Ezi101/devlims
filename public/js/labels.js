...
$(document).ready(function() {
    ...
    $(document).on('click', 'button.labels-preview', function() {
        ...
        var printFormData = $('div.row').find('input[type="checkbox"], input[type="text"]').serialize(); // Serialize printing options form data
        var url = base_path + '/sample/dashbord/labels/preview?' + formData + '&' + printFormData;
        console.log(url);
        window.open(url, 'newwindow');
    });
...
$(document).ready(function() {
    ...
    $('button#labels_preview').click(function() {
        if ($('form#preview_setting_form table#product_table tbody tr').length > 0) {
            var url = base_path + '/issue/sample/labels/preview?' + $('form#preview_setting_form').serialize();
            console.log(url);
            window.open(url, 'newwindow');
