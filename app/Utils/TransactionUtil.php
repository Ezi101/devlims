...
class TransactionUtil extends Util
{
    ...
    public function createSellTransactionforsample_on_store_workflow_and_test_with_sample_issue($business_id, $input, $invoice_total, $user_id, $batch_no, $issue_id,  $uf_data = true)
    {
        ...
        if ($uf_data === 0) {
            ...
            $final_total = isset($input['final_total']) ? $this->num_uf($input['final_total']) : 0;
        }
    ...
    public function createSellTransactionforsample($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        $sale_type = !empty($input['type']) ? $input['type'] : 'sell';
        $invoice_scheme_id = !empty($input['invoice_scheme_id']) ? $input['invoice_scheme_id'] : null;
        ...
        $pay_term_number = isset($input['pay_term_number']) ? $input['pay_term_number'] : null;
        $pay_term_type = isset($input['pay_term_type']) ? $input['pay_term_type'] : null;
        $keysToCheck = ['physical', 'chemical', 'micro'];
        $transaction_ids = [];
        ...
        foreach ($keysToCheck as $key) {
            if (isset($input[$key]) && !empty($input[$key]['batch_no'])) {
                ...
                $current_year = date('y');
                $con = $input[$key]['lab_manager'];
                $issue_id_prefix = $current_month . $current_year . $con;
                ...
                foreach ($input[$key]['batch_no'] as $batch_id) {
                    ...
                    // ✅ FIX 1: Batch exist karta hai aur quantity > 0 hai?
                    // Batch exi
                    $batchRecord = Batch::where('business_id', $business_id)
                        ->where('id', $batch_id)
                        ->first();

                    if (empty($batchRecord) || $batchRecord->quantity <= 0) {
                        continue; // ❌ 0 quantity — skip karo
                    }

                    $transaction_count += 1;

                    // ✅ FIX 2: Dobara query nahi — already fetched record use karo
                    $batch_code = $batchRecord->code;

                    $upce_length = 8;
                    $issue_id = str_pad($transaction_count, $upce_length, '0', STR_PAD_LEFT);
                    $first_product = reset($input[$key]['products']);
                    ...
                    $transaction = Transaction::create([
                        ...
                        'product_type' => $first_product['product_type'] ?? 'sample',
                        'status'                 => $input['status'],
                        'sub_status'             => !empty($input['sub_status']) ? $input['sub_status'] : null,
                        'contact_id'             => $input[$key]['lab_manager'],
                        'customer_group_id'      => !empty($input['customer_group_id']) ? $input['customer_group_id'] : null,
                        'invoice_no'             => $issue_id,
                        'ref_no'                 => '',
                        'source'                 => !empty($input['source']) ? $input['source'] : null,
                    ...
                    foreach ($input[$key]['products'] as $product) {
                        $line[] = new TransactionSellLine([
                            ...
                            'quantity'             => $uf_data ? $this->num_uf($product['quantity']) : $product['quantity'],
                            ...
                            'exp_date'             => !empty($product['exp_date']) ? $product['exp_date'] : null,
                            'sub_unit_id'          => !empty($product['sub_unit_id']) ? $product['sub_unit_id'] : null,
                            'batch_no'             => $batch_id,
                        ]);
                    }
                    $transaction->sell_lines()->saveMany($line);
                }
            }
        }

        // ✅ FIX 3: AuditLogger crash fix — pehle check karo
        if (!empty($transaction_ids)) {
            $lastTransaction = Transaction::find(end($transaction_ids));
            if ($lastTransaction) {
                AuditLogger::log(
                    'issued',
                    'Transaction',
                    '<b>Sample ID: ' . $lastTransaction->product_id .
                        ' (' . $lastTransaction->product->name . ')</b> was <b>issued</b> with <b>Transaction ID: ' .
                        $lastTransaction->id . '</b>'
                );
            }
        }
    ...
    public function createSellTransactionforreagent($business_id, $input, $invoice_total, $user_id, $batch_no, $issue_id,  $uf_data = true)
    {
        // dd($input);
        $sale_type = !empty($input['type']) ? $input['type'] : 'sell';
        $invoice_scheme_id = !empty($input['invoice_scheme_id']) ? $input['invoice_scheme_id'] : null;
        ...
        $transaction = Transaction::create([
            ...
            'product_type' => $input['product_type'],
            'status' => $input['status'],
            'sample_issued_id' => $input['issued_id'],
    ...
    public function createSellTransactionforstandard($business_id, $input, $invoice_total, $user_id, $batch_no, $issue_id,  $uf_data = true)
    {
    ...
    public function createOrUpdateSellLines_on_store_workflow_and_test_with_sample_issue($transaction, $products, $batch_no, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
    ...
    public function createOrUpdateSellLines($transaction, $products, $batch_no, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $customer = User::find($transaction->contact_id);
        $role_name = $moduleUtil->getUserRoleName($customer->id);
        // dd($customer,$role_name);
        ...
        if ($il->show_customer == 1) {
            ...
            $output['customer_name'] = $customer->surname . ' ' . $customer->first_name . ' ' . $customer->last_name;
            $output['customer_id'] =  $customer->id;

            $output['customer_mobile'] = $customer->contact_no;

            if ($receipt_printer_type != 'printer') {
                $output['customer_info'] .= $customer->permanent_address;
                if (!empty($customer->permanent_address)) {
                    $output['customer_info'] .= '<br>';
                }
                $output['customer_info'] .= '<b>' . __('contact.mobile') . '</b>: ' . $customer->contact_no;
                if (!empty($customer->alt_number)) {
                    $output['customer_info'] .= ', ' . $customer->alt_number;
                }
            }
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            $sell_line_relations = ['modifiers', 'sub_unit', 'warranties', 'batch'];
    ...
    public function getListPurchases($business_id)
    {
        ...
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->leftJoin('users as use', 'transactions.contact_id', '=', 'use.id')
            ->leftJoin('users as rec_by_afmsl', 'transactions.rec_by_afmsl', '=', 'rec_by_afmsl.id')
            // ->leftJoin('batch as b_no', 'transactions.batch_no', '=', 'b_no.id')
            ->join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
            ->leftJoin('products as s_name', 'pl.product_id', '=', 's_name.id')
            ->leftJoin('product_generic_name', 's_name.id', '=', 'product_generic_name.product_id')
            ->leftJoin('generic_names as g_name', 'product_generic_name.generic_name_id', '=', 'g_name.id')

            ->leftJoin('brands', 'transactions.brand_id', '=', 'brands.id')->where('transactions.business_id', $business_id)
            ->leftJoin('pharmacopoeias', 's_name.types_of_sample', '=', 'pharmacopoeias.id')
            ->leftJoin('contracts as contract', 'transactions.contract_no', '=', 'contract.id')

            ->join('purchase_lines as pl_bs', function ($join) {
                $join->on('transactions.id', '=', 'pl_bs.transaction_id')  // Join based on transaction_id
                    ->where('transactions.location_id', '=', 5);  // Filter based on location_id from transactions table
            })->leftJoin('s_t_r', function ($join) {
                $join->on('pl_bs.batch_no', '=', 's_t_r.batch_no')
                    ->where('s_t_r.status', '=', 'approved');
            })
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'purchase')
            ->where('transactions.product_type', 'sample')
            // ->where('transactions.location_id', $afmsl_location->id)
            ->select(
                'transactions.id',
                DB::raw("CONCAT(s_name.name, ' (', COUNT(DISTINCT pl_bs.batch_no), ')') as sample_name"),
                DB::raw('GROUP_CONCAT(DISTINCT g_name.name SEPARATOR ", ") as generic_names'),
                // DB::raw("CONCAT(ROUND((SELECT SUM(pl2.quantity) FROM purchase_lines as pl2 WHERE pl2.transaction_id = transactions.id), 0), ' PC(s)') as quantity"),
                // 'BS.name as location_name',
                'transactions.status',
                DB::raw("CASE WHEN transactions.status = 'Forward by AFIMS' THEN transactions.d_fwd_to_afmsl ELSE NULL END as forwarded_to_afmsl_date"),
                DB::raw("CASE WHEN transactions.status = 'Forwarded to 2IC' THEN transactions.d_fwd_to_2ic ELSE NULL END as forwarded_to_2ic_date"),
                DB::raw("CASE WHEN transactions.status = 'Received by AFMSL' THEN transactions.d_rcv_by_afmsl ELSE NULL END as received_by_afmsl_date"),
                // 'brands.name as brand',
                'pharmacopoeias.name as pharmacopoeia',
                'transactions.instalments',
                'contract.type as contract_type',
                // DB::raw("CASE WHEN EXISTS (SELECT 1 FROM transactions AS t WHERE t.type = 'sell' AND t.batch_no = transactions.batch_no AND t.invoice_no IS NOT NULL ) THEN 'Issued' ELSE 'Not Issued' END AS i_status"),
                DB::raw('CASE WHEN s_t_r.status = "approved" THEN "yes" ELSE "no" END as transaction_status'),
                'transactions.transaction_date',
                // DB::raw("(SELECT u.name FROM users WHERE u.id = transactions.created_by) as created_by"),
        ...
        if (auth()->check() && auth()->user()->hasRole('SampleRoom(Afmsl)' . '#' . $business_id)) {
            // Exclude records with the "draft" status
            $purchases->where(function ($query) {
                $query->where('transactions.status', 'Forward by AFIMS')
                    ->orWhere('transactions.status', 'Received by AFMSL');
            });
        }
        if (!empty(request()->days && request()->status == 'Received by AFMSL')) {
            // Exclude records with the "draft" status
            $purchases->where(function ($query) {
        ...
        if (!empty(request()->days && request()->status == 'Forward by AFIMS')) {
            // Exclude records with the "draft" status
            $purchases->where(function ($query) {
                $query->where('transactions.status', 'Forward by AFIMS')
                    ->orWhere('transactions.status', 'Forwarded to 2IC');
            });
        }

        if (auth()->check() && auth()->user()->hasRole('2IC' . '#' . $business_id)) {
            // Include records with both 'Forwarded to 2IC' and other statuses
            $purchases->whereIn('transactions.status', ['Forwarded to 2IC', 'Forward by AFIMS']);
        }

        if (auth()->check() && auth()->user()->hasRole('Issue Authority' . '#' . $business_id)) {
            $purchases->where(DB::raw("CASE WHEN EXISTS (SELECT 1 FROM transactions AS t WHERE t.type = 'sell' AND t.batch_no = transactions.batch_no AND t.invoice_no IS NOT NULL ) THEN 'Issued' ELSE 'Not Issued' END"), 'Not Issued');
        } else {
            // Existing logic for filtering by status if user is not oic
            if (!empty(request()->status)) {
                if (!empty((request()->status) == 'Issued' || (request()->status) == 'Not Issued')) {
                    $purchases->where(DB::raw("CASE WHEN EXISTS (SELECT 1 FROM transactions AS t WHERE t.type = 'sell' AND t.batch_no = transactions.batch_no AND t.invoice_no IS NOT NULL ) THEN 'Issued' ELSE 'Not Issued' END"), request()->status);
                } else {
    ...
    public function getListPurchasereturn($business_id)
    {
        ...
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->select(
                ...
                // DB::raw("(SELECT u.name FROM users WHERE u.id = transactions.created_by) as created_by"),
                DB::raw("CONCAT(COALESCE(u.surname, ''),' ',COALESCE(u.first_name, ''),' ',COALESCE(u.last_name,'')) as created_by"),
                'transactions.not_rec_reason',
                'transactions.return_by_2ic_reason',
            );
        // if (auth()->check() && auth()->user()->hasRole('SampleRoom(Afmsl)' . '#' . $business_id)) {
        //     // Exclude records with the "draft" status
        //     $purchases->where(function ($query) {
        ...
        // }
        if (auth()->check() && auth()->user()->hasRole('SampleRoom(Afmsl)' . '#' . $business_id)) {
            if (!request()->routeIs('purchase.views')) {
                $purchases->where(function ($query) {
                    $query->where('transactions.status', 'Forward by AFIMS')
                        ->orWhere('transactions.status', 'Received by AFMSL');
                });
        ...
        $purchases->groupBy('transactions.not_rec_reason', 'transactions.return_by_2ic_reason');
    ...
    //Reagent Purchase List 
    public function getreagentListPurchases($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->join(
            ...
            ->leftJoin('brands as b', 'transactions.brand_id', '=', 'b.id')
            ->leftJoin(
            ...
            ->leftJoin(
                'batch',
                'transactions.batch_no',
                '=',
                'batch.id'
            )
            ->leftJoin('products as p', 'transactions.product_id', '=', 'p.id')
            ->join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
            ->leftJoin('users as u', 'transactions.created_by', '=', 'u.id')
            ->leftJoin('users as use', 'transactions.contact_id', '=', 'u.id')
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'purchase')
            ...
            ->select(
                ...
                'transactions.status',
                'transactions.contract_no',
                DB::raw("CONCAT(ROUND(SUM(pl.quantity), 0), ' PC(s)') as quantity"),
                'transactions.batch_no',
                'transactions.instalments',
                'transactions.potency',
                ...
                DB::raw("CONCAT(COALESCE(use.surname, ''),' ',COALESCE(use.first_name, ''),' ',COALESCE(use.last_name,'')) as assinged_user"),
    ...
    //Standards Purchase List 
    public function getstandardListPurchases($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->join('business_locations AS BS', 'transactions.location_id', '=', 'BS.id')
            ->leftJoin('transaction_payments AS TP', 'transactions.id', '=', 'TP.transaction_id')
            ...
            ->join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
            ->leftJoin('products as s_name', 'pl.product_id', '=', 's_name.id')
            ->leftJoin('generic_names as g_name', 's_name.generic_name', '=', 'g_name.id')
            ...
            ->select(
                ...
                'transactions.document',
                'transactions.transaction_date',
                DB::raw("CONCAT(s_name.name, ' (', COALESCE(g_name.name, ''), ')') as sample_and_generic_name"),
                'g_name.name as generic_name',
                's_name.name as sample_name',
                'b_no.code as batch_number',
                'transactions.ref_no',
                'b_no.mfg_date as m_date',
                'b_no.expiry_date as e_date',
                DB::raw("CONCAT(CAST(pl.quantity AS UNSIGNED), ' PC(s)') as quantity"),
                'contacts.name',
                // 'contacts.supplier_business_name',
                'transactions.status',
                'transactions.potency',
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->leftJoin('products as p', 'transactions.product_id', '=', 'p.id')
            ->leftJoin('product_generic_name', 'p.id', '=', 'product_generic_name.product_id')
            ->leftJoin('generic_names as g_name', 'product_generic_name.generic_name_id', '=', 'g_name.id')
            ->leftJoin('batch as b', 'transactions.batch_no', '=', 'b.id')
            ...
            ->select(
                ...
                // 'use.username as contact_issue_to',
                DB::raw("CONCAT(COALESCE(use.surname, ''),' ',COALESCE(use.first_name, ''),' ',COALESCE(use.last_name,'')) as contact_issue_to"),
                // DB::raw("CONCAT(p.name, ' (', COALESCE(g_name.name, ''), ')') as sample_and_generic_name"),
                'p.name as sample_name',
                DB::raw('GROUP_CONCAT(DISTINCT g_name.name SEPARATOR ", ") as generic_names'),
                ...
                'contacts.supplier_business_name',
                'transactions.status',
                'tsl.sell_line_note as sell_line_note',
                'transactions.payment_status',
                'transactions.final_total',
                ...
                // DB::raw('COUNT( DISTINCT tsl.quantity) as total_items'),
                'tsl.quantity as total_items',
                ...
                DB::raw('DATEDIFF(NOW(), transactions.transaction_date) as assign_days'),
    ...
    public function getListissueSample($business_id, $sale_type = 'sell')
    {
        ...
        $a = $sells->get();
        // dd($a);
        // dd($sells->toSql(), $sells->getBindings(), $sells->get());
        return $sells;
    }
    /**
