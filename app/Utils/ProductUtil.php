...
class ProductUtil extends Util
{
    ...
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax, $combo_variations = [])
    {
        ...
        $variation_data = [
            ...
            'sub_sku' => $sku ?? $product->sku,
            'default_purchase_price' => isset($purchase_price) ? $this->num_uf($purchase_price) : 0.0000,
            'dpp_inc_tax' => isset($dpp_inc_tax) ? $this->num_uf($dpp_inc_tax) : 0.0000,
            'profit_percent' => isset($profit_percent) ? $this->num_uf($profit_percent) : 0.0000,
            'default_sell_price' => isset($selling_price) ? $this->num_uf($selling_price) : 0.0000,
            'sell_price_inc_tax' => isset($selling_price_inc_tax) ? $this->num_uf($selling_price_inc_tax) : 0.0000,
    ...
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true)
    {
        ...
        if (!$product) {
            ...
            \Log::error("Product not found with ID: {$product_id}");
        ...
        if ($product->enable_stock == 1 && $qty_difference != 0) {
            // Ensure variation is associated with the product
            $variation = Variation::where('product_id', $product_id)
                ->first();
            // dd($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true,$variation,$product);

            // Check if the variation was found
            if (!$variation) {
                \Log::error("Variation not found with ID: {$variation_id} for Product ID: {$product_id}");
                return response()->json(['error' => 'Variation not found'], 404);
            }

            // Get the variation and product variation IDs
            ...
            $variation_location_d = VariationLocationDetails::where('variation_id', $variation_id)
                ...
                ->first();
            // dd($variation_location_d, $product_id, $product_variation_id, $location_id, $variation_id);
    ...
    public function updatesample_in_retentionroom($input, $transaction)
    {
        dd($input, $transaction);
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        ...
        if ($product->enable_stock == 1) {
            ...
            // dd($product_id, $variation_id, $location_id, $new_quantity, $old_quantity,$details,$qty_difference);
        }
     ...
     * @return array
     */
    // public function createOrUpdatePurchaseLines($request, $transaction, $input_data, $batches, $currency_details, $enable_product_editing, $allBatchIdsPresent, $newCreatedBatchQuantities, $before_status = null)
    // {
    //     // dd($batches);
    //     // dd($request->afmsl_location_id,$request->afims_location_id,$request->user_location_id,$transaction, $input_data, $batches, $currency_details, $enable_product_editing, $allBatchIdsPresent, $newCreatedBatchQuantities);
    //     $exchange_rate = !empty($transaction->exchange_rate) ? $transaction->exchange_rate : 1;
    //     foreach ($batches as $i => $data) {
    //         $batchIds = !empty($allBatchIdsPresent) ? $allBatchIdsPresent : [null];
    ...
    //         // // Extract quantities for different locations
    //         // $quantities = [
    //         //     'afmsl' => isset($data['afmsl_qty']) ? $data['afmsl_qty'] : 0,
    //         //     'afims' => isset($data['afims_qty']) ? $data['afims_qty'] : 0,
    //         //     'user'  => isset($data['user_qty']) ? $data['user_qty'] : 0,
    //         // ];

    //         // Extract locations
    //         $locations = [
    //             'afmsl' => isset($request->afmsl_location_id) ? $request->afmsl_location_id : null,
    //             'afims' => isset($request->afims_location_id) ? $request->afims_location_id : null,
    //             'user'  => isset($request->user_location_id) ? $request->user_location_id : null,
    //         ];


    //         $quantities = isset($data['batch_quantity']) ? $data['batch_quantity'] : (isset($newCreatedBatchQuantities[$i]) ? $newCreatedBatchQuantities[$i] : 0);

    //         $quantities = $this->num_f($quantities);


    //         // foreach ($quantities as $key => $new_quantity) {
    //             dd($batches, $quantities);
    //             $new_quantity_f = $this->num_f($new_quantity);
    //             $old_qty = 0;

    //             // Check if the location is set and quantity is greater than 0
    //             if (!isset($locations[$key]) || $new_quantity <= 0) {
    //                 continue;
    //             }
    ...
    //                 $purchase_line = new PurchaseLine();
    //                 $purchase_line->product_id = $batches[1]['product_id'];
    //                 $purchase_line->variation_id = $batches[1]['variation_id'];
    //                 $purchase_line->batch_no = $batchIds[$i] ?? '0';
    //                 $purchase_line->contract_no = $transaction->contract_no;
    //                 $purchase_line->cas_no = $data['cas_no'] ?? null;
    //                 $purchase_line->instalments = $data['instalments'];
    //                 $purchase_line->associated_sample = $data['associated_sample'] ?? null;

    //                 // Increase quantity only if status is received
    //                 if ($transaction->status == 'received By AFMSL') {
    //                     $this->updateProductQuantity($locations[$key], $data['product_id'], $data['variation_id'], $new_quantity_f, 0, $currency_details);
    //                 }
    //             }
    //             // dd($new_quantity);
    //             $purchase_line->quantity = $new_quantity;
    //             $purchase_line->pp_without_discount = ($this->num_uf($data['pp_without_discount'] ?? '0.0000', $currency_details) * $exchange_rate) / $multiplier;
    //             $purchase_line->discount_percent = isset($data['discount_percent']) ? $this->num_uf($data['discount_percent'], $currency_details) : '-';
    //             $purchase_line->purchase_price = isset($data['purchase_price']) ? ($this->num_uf($data['purchase_price'], $currency_details) * $exchange_rate) / $multiplier : '-';
    ...
    //             $purchase_line->tax_id = isset($data['purchase_line_tax_id']) ? $data['purchase_line_tax_id'] : null;
    ...
    //                 }
    //                 $variation_data['pp_without_discount'] = ($this->num_uf($data['pp_without_discount'] ?? '0.0000', $currency_details) * $exchange_rate) / $multiplier;
    ...
    //                     if ($before_status == 'received By AFMSL') {
    //                         $this->decreaseProductQuantity(
    ...
    //                 // Unset purchase order line from purchase lines if exists
    //                 if ($transaction->type == 'purchase_order') {
    ...
    public function createOrUpdatePurchasesLines($transaction, $batches, $currency_details, $enable_product_editing, $allBatchIdsPresent, $newCreatedBatchQuantities, $before_status = null, $input_data)
    {
        $enable_product_editing = 1;
        $product_id = $transaction->product_id;

        $first_batch = reset($batches);
        $defaultVariationId = (isset($first_batch['variation_id']) && !empty($first_batch['variation_id'])) ? $first_batch['variation_id'] : $product_id;

        $exchange_rate = !empty($transaction->exchange_rate) ? $transaction->exchange_rate : 1;

        foreach ($batches as $i => $data) {
            $batchId = isset($allBatchIdsPresent[$i]) ? $allBatchIdsPresent[$i] : '0';
            $updated_purchase_lines = [];
            $updated_purchase_line_ids = [];
            ...
            $new_quantity = isset($data['batch_quantity']) ? $data['batch_quantity'] : (isset($newCreatedBatchQuantities[$i]) ? $newCreatedBatchQuantities[$i] : 0);
            $new_quantity_f = $this->num_f($new_quantity);
            $old_qty = 0;

            $variationId = (isset($data['variation_id']) && !empty($data['variation_id'])) ? $data['variation_id'] : $defaultVariationId;
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                $updated_purchase_line_ids[] = $purchase_line->id;
                $old_qty = $purchase_line->quantity;
                $this->updateProductStock($before_status, $transaction, $product_id, $variationId, $new_quantity, $purchase_line->quantity, $currency_details);
            } else {
                $purchase_line = new PurchaseLine();
                $purchase_line->product_id = $product_id;
                $purchase_line->variation_id = $variationId;
                $purchase_line->batch_no = $batchId;
                $purchase_line->contract_no = $transaction->contract_no;
                $purchase_line->cas_no = $data['cas_no'] ?? null;
                $purchase_line->instalments = $data['instalments'] ?? null;
                $purchase_line->associated_sample = $data['associated_sample'] ?? null;

                if (strtolower($transaction->status) == 'received by afmsl') {
                    $this->updateProductQuantity($transaction->location_id, $product_id, $variationId, $new_quantity_f, 0, $currency_details);
                }
            }
            ...
            if (!empty($updated_purchase_line_ids)) {
                ...
                if ($delete_purchase_lines->count()) {
                    foreach ($delete_purchase_lines as $delete_purchase_line) {
                        $delete_purchase_line_ids[] = $delete_purchase_line->id;
                        if (strtolower($before_status) == 'received by afmsl') {
                            $this->decreaseProductQuantity(
                                $delete_purchase_line->product_id,
                                $delete_purchase_line->variation_id,
                                $transaction->location_id,
    ...
    public function createOrUpdatePurchaseLines($request, $transaction, $input_data, $batches, $currency_details, $enable_product_editing, $allBatchIds, $allBatchIdsPresent, $newCreatedBatchQuantities, $before_status = null)
    {
        $enable_product_editing = 1;
        // dd($transaction, $input_data, $batches, $currency_details, $enable_product_editing, $allBatchIdsPresent, $newCreatedBatchQuantities);
        ...
        foreach ($batches as $i => $data) {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                $purchase_line = new PurchaseLine();
                $purchase_line->product_id = $batches[1]['product_id'];
                // dd($purchase_line->product_id);
                $purchase_line->variation_id = $batches[1]['variation_id'];

                $purchase_line->batch_no = count($regularBatchIds) > 0 ? array_shift($regularBatchIds) : null;
            ...
            // Update purchase order line quantity received
            $this->updatePurchaseOrderLine($purchase_line->purchase_order_line_id, $purchase_line->quantity, $old_qty);
            // dd($batchIds,$data,$enable_product_editing , $transaction->type == 'purchase');
        ...
        // dd( $purchase_line->w_batch_no ,  $purchase_line->batch_no);

        return $delete_purchase_lines;
    }
    //
    //     public function UpdatePurchaseLinesOnly($request, $transaction, $input_data, $batches, $currency_details, $enable_product_editing, $allBatchIdsPresent, $newCreatedBatchQuantities, $before_status = null, $c_tran_id)
    //     {
    //         //  dd($allBatchIdsPresent,$newCreatedBatchQuantities);
    //         $enable_product_editing = 1;
    //         // dd($transaction, $input_data, $batches, $currency_details, $enable_product_editing, $allBatchIdsPresent, $newCreatedBatchQuantities);
    //
    //         $exchange_rate = !empty($transaction->exchange_rate) ? $transaction->exchange_rate : 1;
    ...
    // //             dd($new_quantity_f);
    //             $old_qty = 0;
    //             if (isset($data['purchase_line_id']) && isset($data['batch_id'])) {
    //
    //                 $count = count($data['purchase_line_id']);
    // //                 dd($count);
    // //                dd(33);
    //                 for ($i = 0; $i < $count; $i++) {
    //                     $id = $data['purchase_line_id'][$i];
    //
    //                     $purchase_line = PurchaseLine::findOrFail($id);
    // //dd($id);
    //
    //
    // //                        $purchase_line->instalments = $data['instalments']; // Update instalments
    // //                    $purchase_line->save(); // Persist changes to the database
    //
    //                     $updated_purchase_line_ids[] = $purchase_line->id;
    //
    //
    ...
    //                     //                     $transaction->purchase_lines()->save($updated_purchase_line_ids);
    //                     //                 }
    //
    //
    // //                    dd($old_qty,$new_quantity);
    //
    // //                    $this->updateProductStock($before_status, $transaction, $data['product_id'], $data['variation_id'], $new_quantity, $purchase_line->quantity, $currency_details);
    ...
    //
    //                 }
    //             //        $purchase_line->product_id = $batches[1]['product_id'];  // Assuming `product_id` is available
    //             // $purchase_line->variation_id = $batches[1]['variation_id']; // Assuming `variation_id` is available
    //             // $purchase_line->batch_no = $batchIds[$i] ?? '0'; // Assuming `batch_no` is available from $batchIds
    //             // $purchase_line->contract_no = $transaction->contract_no;  // Assuming `contract_no` is available
    //             // $purchase_line->cas_no = $data['cas_no'] ?? null;
    //             // $purchase_line->instalments = $data['instalments'] ?? null;
    //             // $purchase_line->associated_sample = $data['associated_sample'] ?? null;
    //             // $purchase_line->transaction_id = $c_tran_id ?? null;
    //
    ...
    // //        dd($updated_purchase_lines);
    // //dd($data);
    // //         dd( $purchase_line->instalments->all());
    // // dd( $delete_purchase_lines);
    //         return $delete_purchase_lines;
    //     }


    // function for update purchae lne

    public function UpdatePurchaseLinesOnly($request, $transaction, $input_data, $batches, $currency_details, $enable_product_editing, $allBatchIdsPresent, $newCreatedBatchQuantities, $before_status = null, $purchases)
    {
        // Pass all batches to the product utility function for processing
        $updated_purchase_lines = $this->processBatchUpdates($batches, $transaction, $currency_details, $allBatchIdsPresent, $newCreatedBatchQuantities, $before_status);

        return $updated_purchase_lines;
    }

    private function processBatchUpdates($batches, $transaction, $currency_details, $allBatchIdsPresent, $newCreatedBatchQuantities, $before_status)
    {
        // dd($allBatchIdsPresent,$newCreatedBatchQuantities);
        // Prepare an array to collect the updated purchase lines
        $updated_purchase_lines = [];

        // Check if there are any batches and extract product_id and variation_id from the very first batch
        if (!empty($batches)) {
            // Extract from the first batch
            $product_id = isset($batches[1]['product_id']) ? $batches[1]['product_id'] : null;
            $variation_id = isset($batches[1]['variation_id']) ? $batches[1]['variation_id'] : null;
            //dd($product_id,$variation_id);
            // Validate that both product_id and variation_id exist


            // Loop through each batch for processing
            foreach ($batches as $i => $data) {
                // Update or create purchase lines for this batch
                $purchase_lines = $this->updateOrCreatePurchaseLine($data, $transaction, $currency_details, $allBatchIdsPresent, $newCreatedBatchQuantities, $i, $before_status, $product_id, $variation_id);

                // Append the returned lines for this batch to the updated_purchase_lines
    ...
    private function updateOrCreatePurchaseLine($data, $transaction, $currency_details, $allBatchIdsPresent, $newCreatedBatchQuantities, $i, $before_status, $product_id, $variation_id)
    {
        // dd($data);
        // Initialize the return array for this batch
        $updated_purchase_lines = [];

        //        // Get exchange rate, multiplier, and batch quantity
        // $exchange_rate = !empty($transaction->exchange_rate) ? $transaction->exchange_rate : 1;
        //$multiplier = 1;
        if (!empty($data['sub_unit_id'])) {
            $unit = Unit::find($data['sub_unit_id']);
        ...
        // Get the new quantity from the data or created quantities
        $new_quantity = isset($data['batch_quantity']) ? $data['batch_quantity'] : (isset($newCreatedBatchQuantities[$i]) ? $newCreatedBatchQuantities[$i] : 0);
        //         dd($new_quantity);
        $new_quantity_f = $this->num_f($new_quantity);

        // Find the corresponding batch ID
        $batchIds = !empty($allBatchIdsPresent) ? $allBatchIdsPresent : [null];
        //        dd($batchIds);
        $batch_id = $data['mapped_batch_id'] ?? ($batchIds[$i] ?? null);
        //        dd($batchIds,$batch_id,$i,$transaction->id);
        //dd($transaction->id);
        // Check if purchase lines exist for this batch under the same transaction
        // Pehle purchase_line_id se directly update karo
        // Pehle wala
        if (!empty($data['purchase_line_id'])) {
            $existing_purchase_lines = PurchaseLine::where('id', $data['purchase_line_id'])->get();
        } else {
            $existing_purchase_lines = PurchaseLine::where('transaction_id', $transaction->id)
        ...
        // dd($existing_purchase_lines);
        //dd($transaction->id,$batch_id);
        // If purchase lines exist, update them
        if ($existing_purchase_lines->isNotEmpty()) {
            foreach ($existing_purchase_lines as $purchase_line) {
                //                dd($new_quantity);
                $purchase_line->quantity = $new_quantity;

                $purchase_line->product_id = $product_id;  // Use the product_id from the first batch
                $purchase_line->batch_no     = $data['mapped_batch_id'] ?? $batch_id ?? '0';
                $purchase_line->variation_id = $variation_id;
                $purchase_line->instalments = $data['instalments'] ?? null;
                $purchase_line->purchase_price = isset($data['purchase_price'])
                    ? ($this->num_uf($data['purchase_price'], $currency_details) * $exchange_rate) / $multiplier
                    : 0;
                $purchase_line->purchase_price_inc_tax = isset($data['purchase_price_inc_tax'])
                    ? ($this->num_uf($data['purchase_price_inc_tax'], $currency_details) * $exchange_rate) / $multiplier
                    : 0;
                $purchase_line->item_tax = isset($data['item_tax'])
                    ? ($this->num_uf($data['item_tax'], $currency_details) * $exchange_rate) / $multiplier
                    : 0;
                //                $purchase_line->save();
            ...
            //            dd(25);
            // If no existing purchase lines, create new ones for this batch
            $purchase_line = new PurchaseLine();
            $purchase_line->product_id = $product_id;

            $purchase_line->variation_id = $variation_id;
            // dd($batchIds[$i]);
            $purchase_line->batch_no     = $data['mapped_batch_id'] ?? $batchIds[$i] ?? '0';
            $purchase_line->quantity = $new_quantity;
            $purchase_line->contract_no = $transaction->contract_no;
            // dd($purchase_line->contract_no);
            $purchase_line->cas_no = $data['cas_no'] ?? null;
    ...
    public function updateProductStock($status_before, $transaction, $product_id, $variation_id, $new_quantity, $old_quantity, $currency_details)
    {
        ...
        if (strtolower($status_before) == 'received by afmsl' && strtolower($transaction->status) == 'received by afmsl') {
        ...
        } elseif (strtolower($status_before) == 'received by afmsl' && strtolower($transaction->status) != 'received by afmsl') {
    ...
    public function filterstandard($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ->active()
            ->whereNull('variations.deleted_at')
    ...
    public function filterreagent($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ->active()
            ->whereNull('variations.deleted_at')
            ->where('products.product_type', 'reagent')
            ->leftjoin('units as U', 'products.unit_id', '=', 'U.id')
            ->leftjoin(
                'variation_location_details AS VLD',
                function ($join) use ($location_id) {
