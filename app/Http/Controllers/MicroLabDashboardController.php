...
class MicroLabDashboardController extends Controller
{
    /**
    ...
    {
        $microLabManagers = Role::whereIn('name', [
            'Micro Lab Manager#15'
        ])->with('users')->first();

        $microUsers = User::join('model_has_roles', 'model_has_roles.model_id', '=', 'users.id')
            ->where('model_has_roles.role_id', $microLabManagers->id)
            ->select('users.*')->get();

        $announcement = Announcement::latest()->take(10)->whereIn('created_by', $microUsers->pluck('id'))->get();

        $task = SampleReading::latest('updated_at')
            ->take(20)
            ->with(['testmethod', 'samples', 'task' => function ($query) use ($microUsers) {
                $query->whereIn('created_by', $microUsers->pluck('id'));
            }])
            ->whereHas('task', function ($query) use ($microUsers) {
                $query->whereIn('created_by', $microUsers->pluck('id'));
            })
            ->get();
        ...
        $data = ProjectTask::with('createdBy')->whereIn('created_by', $microUsers->pluck('id'))->get();
        ...
        $taskData = ProjectTask::latest('updated_at')->whereIn('created_by', $microUsers->pluck('id'))->get();
