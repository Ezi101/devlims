...
class PhysicalLabDashboardController extends Controller
{
      /**
    ...
    public function getData(Request $request)
    {
        ...
        $physicalUsers = User::join('model_has_roles','model_has_roles.model_id','=','users.id')
            ->where('model_has_roles.role_id', $physicalLabManager->id)
            ->select('users.*')->get();
    
        $physicalUserIds = $physicalUsers->pluck('id')->toArray();
    
        $announcement = Announcement::latest()->take(10)->whereIn('created_by', $physicalUserIds)->get();
    
        $task = SampleReading::latest('updated_at')
            ->take(20)
            ->with(['testmethod', 'samples', 'task' => function($query) use ($physicalUserIds) {
                $query->whereIn('created_by', $physicalUserIds);
            }])
            ->whereHas('task', function($query) use ($physicalUserIds) {
                $query->whereIn('created_by', $physicalUserIds);
            })
            ->get();
        ...
        $data = ProjectTask::with('createdBy')->whereIn('created_by', $physicalUserIds)->get();
        ...
        $taskData = ProjectTask::latest('updated_at')->whereIn('created_by', $physicalUserIds)->get();
        ...
        return response()->json(['success' => true, 'announcement' => $announcement, 'task' => $task,
            'total' => $total, 'completed' => $completed,
            'not_started' => $not_started, 'in_progress' => $in_progress, 'on_hold' => $on_hold, 'cancelled' => $cancelled,
            'totalToday' => $totalToday, 'completedToday' => $completedToday,
            'not_startedToday' => $not_startedToday, 'in_progressToday' => $in_progressToday, 'on_holdToday' => $on_holdToday, 'cancelledToday' => $cancelledToday,
