...
class ReportController extends Controller
{
    ...
    //     $ReceivedSampleInst1 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('product_type', 'sample')->where('instalments', 'instalments_1')->count('product_type', 'sample');
    //     $ReceivedSampleInst2 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('product_type', 'sample')->where('instalments', 'instalments_2')->count('product_type', 'sample');
    //     $ReceivedSampleInst3 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('product_type', 'sample')->where('instalments', 'instalments_3')->count('product_type', 'sample');
    //     $ReceivedSampleInst4 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('product_type', 'sample')->where('instalments', 'instalments_4')->count('product_type', 'sample');

    //     $ReceivedBatchInst1 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('instalments', 'instalments_1')->count('batch_no');
    //     $ReceivedBatchInst2 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('instalments', 'instalments_2')->count('batch_no');
    //     $ReceivedBatchInst3 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('instalments', 'instalments_3')->count('batch_no');
    //     $ReceivedBatchInst4 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('instalments', 'instalments_4')->count('batch_no');

    //     $TotalReceivedSamples = $ReceivedSampleInst1 + $ReceivedSampleInst2 + $ReceivedSampleInst3 + $ReceivedSampleInst4;
    //     $TotalReceivedBatchs = $ReceivedBatchInst1 + $ReceivedBatchInst2 + $ReceivedBatchInst3 + $ReceivedBatchInst4;

    //     $ReceivedSTRSample1 = 10;
    //     $ReceivedSTRSample2 = 5;
    //     $ReceivedSTRSample3 = 3;
    //     $ReceivedSTRSample4 = 2;
    ...
    //     $TotalSTRSamples = $ReceivedSTRSample1 + $ReceivedSTRSample2 + $ReceivedSTRSample3 + $ReceivedSTRSample4;
    //     $TotalSTRBatchs = $ReceivedSTRBatch1 + $ReceivedSTRBatch2 + $ReceivedSTRBatch3 + $ReceivedSTRBatch4;

    //     //Subtracte from Received Sample and STR Sample from instalments 1 ,2 ,3 ,4
    //     $TRSTRI1 = $ReceivedSampleInst1 - $ReceivedSTRSample1;
    //     $TRSTRI2 = $ReceivedSampleInst2 - $ReceivedSTRSample2;
    //     $TRSTRI3 = $ReceivedSampleInst3 - $ReceivedSTRSample3;
    //     $TRSTRI4 = $ReceivedSampleInst4 - $ReceivedSTRSample4;

    //     //Subtracte from Received Batch and STR Batch from instalments 1 ,2 ,3 ,4
    //     $TRBSTRI1 = $ReceivedBatchInst1 - $ReceivedSTRBatch1;
    //     $TRBSTRI2 = $ReceivedBatchInst2 - $ReceivedSTRBatch2;
    //     $TRBSTRI3 = $ReceivedBatchInst3 - $ReceivedSTRBatch3;
    //     $TRBSTRI4 = $ReceivedBatchInst4 - $ReceivedSTRBatch4;

    //     // Total pending
    //     $totalSamplePending = $TRSTRI1 + $TRSTRI2 + $TRSTRI3 + $TRSTRI4;
    //     $totalBatchPending = $TRBSTRI1 + $TRBSTRI2 + $TRBSTRI3 + $TRBSTRI4;
    //     return view('report.sample_report',
    //         compact('ReceivedSampleInst1', 'ReceivedSampleInst2', 'ReceivedSampleInst3', 'ReceivedSampleInst4',
    //             'ReceivedBatchInst1', 'ReceivedBatchInst2', 'ReceivedBatchInst3', 'ReceivedBatchInst4',
    //             'TotalReceivedSamples', 'TotalReceivedBatchs', 'TotalSTRSamples', 'TotalSTRBatchs', 'totalSamplePending', 'totalBatchPending',
    //             'ReceivedSTRBatch1', 'ReceivedSTRBatch2', 'ReceivedSTRBatch3', 'ReceivedSTRBatch4', 'ReceivedSTRSample1', 'ReceivedSTRSample2', 'ReceivedSTRSample3', 'ReceivedSTRSample4',
    //             'TRSTRI1', 'TRSTRI2', 'TRSTRI3', 'TRSTRI4', 'TRBSTRI1', 'TRBSTRI2', 'TRBSTRI3', 'TRBSTRI4'));

    // }

    public function sample_report_pdf(Request $request)
    {
        ...
        $ReceivedSampleInst1 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('product_type', 'sample')->where('instalments', 'instalments_1')->count();
        $ReceivedSampleInst2 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('product_type', 'sample')->where('instalments', 'instalments_2')->count();
        $ReceivedSampleInst3 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('product_type', 'sample')->where('instalments', 'instalments_3')->count();
        $ReceivedSampleInst4 = Transaction::Where('business_id', $business_id)->where('status', 'received')->where('product_type', 'sample')->where('instalments', 'instalments_4')->count();
        ...
        $receivedSTRSampleCount = STR::where('business_id', $business_id)->count();

        $ReceivedSTRSample1 = STR::where('business_id', $business_id)
            ->whereNotNull('sample_id')
            ->whereNotNull('batch_no')
            ->select('batch_no', 'sample_id')
            ->first();

        if ($ReceivedSTRSample1) {
            $str1 = Transaction::where('business_id', $business_id)->where('product_id', $ReceivedSTRSample1->sample_id)
                ->where('status', 'received')
                ->where('batch_no', $ReceivedSTRSample1->batch_no)
                ->where('product_type', 'sample')
                ->where('instalments', 'instalments_1')
                ->count();
            // dd($ReceivedSTRSample1,$str1);

            $str2 = Transaction::where('business_id', $business_id)->where('product_id', $ReceivedSTRSample1->sample_id)
                ->where('status', 'received')
                ->where('batch_no', $ReceivedSTRSample1->batch_no)
                ->where('product_type', 'sample')
                ->where('instalments', 'instalments_2')
                ->count();
            $str3 = Transaction::where('business_id', $business_id)->where('product_id', $ReceivedSTRSample1->sample_id)
                ->where('status', 'received')
                ->where('batch_no', $ReceivedSTRSample1->batch_no)
                ->where('product_type', 'sample')
                ->where('instalments', 'instalments_3')
                ->count();
            $str4 = Transaction::where('business_id', $business_id)->where('product_id', $ReceivedSTRSample1->sample_id)
                ->where('status', 'received')
                ->where('batch_no', $ReceivedSTRSample1->batch_no)
                ->where('product_type', 'sample')
                ->where('instalments', 'instalments_3')
            ...
            $strbatch1 = Transaction::where('business_id', $business_id)->where('product_id', $ReceivedSTRSample1->sample_id)
                ->where('status', 'received')
                ->where('batch_no', $ReceivedSTRSample1->batch_no)
                ->where('product_type', 'sample')
                ->where('instalments', 'instalments_1')
                ->count(['batch_no']);
            $strbatch2 = Transaction::where('business_id', $business_id)->where('product_id', $ReceivedSTRSample1->sample_id)
                ->where('status', 'received')
                ->where('batch_no', $ReceivedSTRSample1->batch_no)
                ->where('product_type', 'sample')
                ->where('instalments', 'instalments_2')
                ->count(['batch_no']);
            $strbatch3 = Transaction::where('business_id', $business_id)->where('product_id', $ReceivedSTRSample1->sample_id)
                ->where('status', 'received')
                ->where('batch_no', $ReceivedSTRSample1->batch_no)
                ->where('product_type', 'sample')
                ->where('instalments', 'instalments_3')
                ->count(['batch_no']);
            $strbatch4 = Transaction::where('business_id', $business_id)->where('product_id', $ReceivedSTRSample1->sample_id)
                ->where('status', 'received')
                ->where('batch_no', $ReceivedSTRSample1->batch_no)
                ->where('product_type', 'sample')
                ->where('instalments', 'instalments_3')
            ...
            $TotalSTRSamples = $str1 + $str2 + $str3 + $str4;
            $TotalSTRBatchs = $strbatch1 + $strbatch2 + $strbatch3 + $strbatch4;

            //Subtracte from Received Sample and STR Sample from instalments 1 ,2 ,3 ,4
            $TRSTRI1 = $ReceivedSampleInst1 - $str1;
            $TRSTRI2 = $ReceivedSampleInst2 - $str2;
            $TRSTRI3 = $ReceivedSampleInst3 - $str3;
            $TRSTRI4 = $ReceivedSampleInst4 - $str4;

            //Subtracte from Received Batch and STR Batch from instalments 1 ,2 ,3 ,4
            $TRBSTRI1 = $ReceivedBatchInst1 - $strbatch1;
            $TRBSTRI2 = $ReceivedBatchInst2 - $strbatch2;
            $TRBSTRI3 = $ReceivedBatchInst3 - $strbatch3;
            $TRBSTRI4 = $ReceivedBatchInst4 - $strbatch4;
    ...
    public function search_supplier_report()
    {
        ...
        if (request()->ajax()) {
            ...
            if (!empty(request()->report_id)) {
                $purchases->where('s_name.name', request()->report_id);
            }
            if (!empty(request()->contract_type)) {
                $purchases->where('contract.type', request()->contract_type);
            }
            if (!empty(request()->contract_no)) {
                $purchases->where('contract.id', request()->contract_no);
            }
            if (!empty(request()->instalment)) {
                $purchases->where('transactions.instalments', request()->instalment);
            }

            $purchases->orderByRaw("SUBSTRING(sample_and_generic_name FROM 1 FOR 1)");
    ...
    public function search_supplier_report_samples($supplier_id)
    {
        $business_id = request()->session()->get('user.business_id');
        $purchases = $this->transactionUtil->getListPurchases($business_id);
    ...
    public function serach_contract_no($contract_no)
    {
        ...
        if (!empty($contract_no)) {
            $purchases->where('contract.type', $contract_no);
        }
        return response()->json($purchases->get());
    }
    public function serach_instalments($contract_no_ins)
    {
        $business_id = request()->session()->get('user.business_id');
        $purchases = $this->transactionUtil->getListPurchases($business_id);

        if (!empty($contract_no_ins)) {
            $purchases->where('transactions.contract_no', $contract_no_ins);
        }
        return response()->json($purchases->get());
    }

    // public function selectTender(Request $request)
    // {
    ...
    //         ->where('contracts.type', 'tender')
    //         ->leftJoin('transactions', 'contracts.sample_id', '=', 'transactions.product_id')
    ...
    //             'contracts.sample_id',
    //             DB::raw("IFNULL(GROUP_CONCAT(DISTINCT contacts.supplier_business_name SEPARATOR '<br>'), 'N/A') as supplier_names")
    //         ])
    //         ->groupBy('contracts.id',)
    //         ->havingRaw('supplier_names IS NOT NULL');


    //     if ($no_of_days) {
    //         $startDate = Carbon::now()->subDays($no_of_days);
    //         $query->whereDate('contracts.created_at', '>=', $startDate);
    //     }
    //     $contracts = $query->get();
    //     $queuedCount = 0;
    //     $inProgressCount = 0;
    //     $completedCount = 0;
    //     $filteredTenders = collect();
    ...
    //                 $contract->statuses = 'queued';
    //                 $queuedCount++;
    //             } elseif (count($statuses) === count(array_filter($statuses, function ($status) {
    //                 return strtolower(trim($status)) === 'approved';
    //             }))) {
    //                 $contract->statuses = 'completed';
    //                 $completedCount++;
    //             } else {
    //                 $contract->statuses = 'inprogress';
    //                 $inProgressCount++;
    //             }
    //         }
    ...
    //             'completed'   => 'Completed',
    //         ];
    //         $contract->statuses = $statusMapping[$contract->statuses] ?? $contract->statuses;

    //         if (!$status || $status === $contract->statuses) {
    //             $filteredTenders->push($contract);
    //         }
    //     }
    //     $tenderIds = $filteredTenders->pluck('id')->toArray();
    //     foreach ($filteredTenders as $tender) {
    //         $tenders[] = [
    //             'number' => $tender->number,
    //         ];
    //         $samplesForTender = \App\Contract::where('number', $tender->number)
    //             ->pluck('sample_id')
    //             ->toArray();
    //         $sampleIds[$tender->number] = $samplesForTender;
    //     }
    //     $productsData = \DB::table('contracts')
    //         ->leftJoin('transactions', 'contracts.sample_id', '=', 'transactions.product_id')
    ...
    //         ->leftJoin('s_t_r', 's_t_r.sample_id', '=', 'transactions.product_id')
    //         ->leftJoin('brands', 'products.brand_id', '=', 'brands.id')
    //         ->select([
    //             'contracts.id as contract_id',
    //             'contracts.number as contract_number',
    //             'contracts.sample_id as sample_id',
    //             'products.name as product_name',
    //             'batch.code as batch_code',
    //             'transactions.instalments',
    //             'transactions.status',
    //             'purchase_lines.quantity as batch_quantity',
    //             'transactions.created_at as transaction_date',
    //             'contacts.supplier_business_name as supplier_names',
    //             \DB::raw("GROUP_CONCAT(generic_names.name SEPARATOR '<br>') as generic_name"),
    //             's_t_r.str_no as str_no',
    //             's_t_r.rejected_by as str_remarks',
    //             'brands.name as forim',
    //         ])
    //         ->leftJoin('product_generic_name', 'products.id', '=', 'product_generic_name.product_id')
    //         ->leftJoin('generic_names', 'product_generic_name.generic_name_id', '=', 'generic_names.id')
    //         ->where('transactions.status', 'Received by AFMSL')
    //         ->whereIn('contracts.id', $tenderIds)
    //         ->groupBy('contracts.id')
    //         ->get();
    //     foreach ($productsData as $product) {
    //         foreach ($sampleIds as $tenderNumber => $sampleIdArray) {
    //             $productObj = Product::find($product->sample_id);
    //             if ($productObj) {
    //                 $product->specs = $productObj->pharma->name ?? '';
    //             }
    //             if (in_array($product->sample_id, $sampleIdArray)) {
    //                 $finalData[$tenderNumber][] = $product;
    //             }
    //         }
    //     }
    //     $totalCount = $filteredTenders->count();
    //     if ($request->ajax()) {
    //         return response()->json([
    //             'filteredTenders' => $finalData,
    ...
    //         ]);
    //     } else {
    //         return view('report.supplier', compact('finalData', 'queuedCount', 'inProgressCount', 'completedCount', 'totalCount'));
    //     }
    // }
    public function selectTender(Request $request)
    {
        ...
        $query = Contract::where('contracts.business_id', $business_id)
            ...
            ->groupBy('contracts.id', 'contracts.number', 'contracts.created_at', 'contracts.sample_id')
            ->having('supplier_names', '!=', 'N/A');

        if ($no_of_days) {
            $query->whereDate('contracts.created_at', '>=', Carbon::now()->subDays($no_of_days));
        }

        $contracts     = $query->get();
        $sampleIds_arr = $contracts->pluck('sample_id')->toArray();

        // Step 2: Batches — ek baar fetch (N+1 fix)
        $allBatches  = Batch::whereIn('sample_id', $sampleIds_arr)
            ->select('id', 'sample_id')
            ->get()
            ->groupBy('sample_id');

        $allBatchIds = $allBatches->flatten()->pluck('id')->toArray();

        // Step 3: STR Statuses — ek baar fetch (N+1 fix)
        $allStatuses = collect();
        if (!empty($allBatchIds)) {
            $allStatuses = STR::whereIn('batch_no', $allBatchIds)
                ->select('batch_no', 'status')
                ->get()
                ->groupBy('batch_no');
        }

        // Step 4: Status calculate
        $queuedCount = $inProgressCount = $completedCount = 0;
        $filteredTenders = collect();

        foreach ($contracts as $contract) {
            $batches  = $allBatches->get($contract->sample_id, collect());
            ...
            if (empty($batchIds)) {
                ...
                $statuses = collect($batchIds)
                    ->flatMap(fn($bid) => $allStatuses->get($bid, collect()))
                    ->pluck('status')
                    ->toArray();
                ...
                if (empty($statuses)) {
                    $contractStatus = 'Queued';
                    $queuedCount++;
                } elseif (count($statuses) === count(array_filter($statuses, fn($s) => strtolower(trim($s)) === 'approved'))) {
            ...
            $contract->statuses = $contractStatus;

            if (!$status || $status === $contractStatus) {
                $filteredTenders->push($contract);
            }
        }
        ...
        // Step 5: Products data — s_t_r join nahi (duplication fix)
        $productsData = DB::table('contracts')
            ->leftJoin('transactions', 'contracts.sample_id', '=', 'transactions.product_id')
            ->leftJoin('products', 'transactions.product_id', '=', 'products.id')
            ->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->leftJoin('product_generic_name', 'products.id', '=', 'product_generic_name.product_id')
            ->leftJoin('generic_names', 'product_generic_name.generic_name_id', '=', 'generic_names.id')
            ->leftJoin('pharmacopoeias', 'products.types_of_sample', '=', 'pharmacopoeias.id')
            ->select([
                'contracts.id as contract_id',
                'contracts.number as contract_number',
                'contracts.sample_id as sample_id',
                ...
                'transactions.created_at as transaction_date',
                'contacts.supplier_business_name as supplier_names',
                DB::raw("GROUP_CONCAT(DISTINCT generic_names.name SEPARATOR '<br>') as generic_name"),
                'brands.name as forim',
                'pharmacopoeias.name as specs',
            ])
            ->where('transactions.status', 'Received by AFMSL')
            ->whereIn('contracts.id', $tenderIds)
            ->groupBy(
        ...
        // Step 6: STR alag fetch — (duplication fix)
        $sampleIdsList = $productsData->pluck('sample_id')->unique()->toArray();
        $strData = DB::table('s_t_r')
            ->whereIn('sample_id', $sampleIdsList)
            ->select('sample_id', 'str_no', 'rejected_by')
            ->get()
            ->keyBy('sample_id');

        $productsData = $productsData->map(function ($product) use ($strData) {
            $str = $strData->get($product->sample_id);
            $product->str_no      = $str->str_no ?? null;
            $product->str_remarks = $str->rejected_by ?? null;
            return $product;
        });

        // Step 7: sampleIds map
        $sampleIdsMap = $filteredTenders->groupBy('number')
            ->map(fn($group) => $group->pluck('sample_id')->toArray());

        // Step 8: finalData
        $finalData = [];
        foreach ($productsData as $product) {
            foreach ($sampleIdsMap as $tenderNumber => $sampleIdArray) {
                if (in_array($product->sample_id, $sampleIdArray)) {
                    $finalData[$tenderNumber][] = $product;
                }
            }
    ...
    public function getSupplierReport(Request $request)
    {
        ...
        // Fetch the suppliers and products
        $suppliers = \App\Contact::where('type', 'supplier')->pluck('supplier_business_name', 'id');
        $products = \App\Product::where('product_type', 'sample')->pluck('name', 'id');

        // Initialize contract_numbers as an empty array
        $contract_numbers = [];

        // Check if contract_type is provided in the request and fetch contract numbers
        if ($request->contract_type) {
            $contract_numbers = \App\Contract::where('type', $request->contract_type)
                ->pluck('number', 'id')
                ->toArray();
        }

        // Build the query based on the filters
        $query = \App\Contract::where('contracts.business_id', $business_id)
            ->leftJoin('transactions', 'contracts.sample_id', '=', 'transactions.product_id')
            ->leftJoin('products', 'transactions.product_id', '=', 'products.id')
            ->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->leftJoin('purchase_lines', 'purchase_lines.transaction_id', '=', 'transactions.id')
            ->leftJoin('batch', 'purchase_lines.batch_no', '=', 'batch.id')
            // Joining str table to fetch STR details
            ->leftJoin('s_t_r', 's_t_r.sample_id', '=', 'transactions.product_id') // Assuming 'product_id' in str table
            ->select([
                'contracts.id as contract_id',
                'contracts.number as contract_number',
                ...
                'transactions.created_at as transaction_date',
                'contacts.supplier_business_name as supplier_names',
                // \DB::raw('GROUP_CONCAT(generic_names.name,<br>) as generic_name'),
                's_t_r.str_no as str_no',
            ])->leftJoin('product_generic_name', 'products.id', '=', 'product_generic_name.product_id') // Join pivot table
            ->leftJoin('generic_names', 'product_generic_name.generic_name_id', '=', 'generic_names.id') // Join generic_names table

            // Filter by the tender IDs we have already filtered
            ->groupBy('contracts.id');
        ...
        if ($request->supplier) {
            $query->where('contacts.id', $request->supplier);
        }

        if ($request->sample) {
            $query->where('transactions.product_id', $request->sample);
        }

        if ($request->instalments) {
            $query->where('transactions.instalments', $request->instalments);
        }

        if ($request->contract_no) {
            $query->where('transactions.contract_no', $request->contract_no);
        }

        if ($request->has('tenders_id')) {
            $query->where('contracts.id', $request->tenders_id); // Filter by tenders_id
        }
        ...
        $batchCount = $data->sum('batch_quantity');
