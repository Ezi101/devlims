...
class PurchaseReturnController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            if ($permitted_locations != 'all') {
                $purchases_returns = $purchases_returns->whereIn('transactions.location_id', $permitted_locations);
            }

            if (!empty(request()->location_id)) {
                $purchases_returns = $purchases_returns->where('transactions.location_id', request()->location_id);
            }

            if (!empty(request()->supplier_id)) {
                $supplier_id = request()->supplier_id;
                $purchases_returns = $purchases_returns->where('contacts.id', $supplier_id);
            }
            if (!empty(request()->start_date) && !empty(request()->end_date)) {
                $start = request()->start_date;
                $end = request()->end_date;
                $purchases_returns = $purchases_returns->whereDate('transactions.transaction_date', '>=', $start)
    ...
    public function add($id)
    {
        ...
        $purchase = Transaction::where('business_id', $business_id)
            ...
            ->where('return_parent_id', $id)->first();
    ...
    public function store(Request $request)
    {
        ...
        return redirect('receive-stock-return')->with('status', $output);
    }
